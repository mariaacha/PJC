...
public class TIntDoubleHashMap extends TIntDoubleHash implements TIntDoubleMap, Serializable, Cloneable {
     ...
     */
    public TIntDoubleHashMap (int[] keys, double[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TIntDoubleHashMap(TIntDoubleMap map, int numProcs) {
        super(map.size());
        if (map instanceof TIntDoubleHashMap) {
            TIntDoubleHashMap hashmap = (TIntDoubleHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TIntDoubleHashMap clone() {
        TIntDoubleHashMap result;
        try {
            result = (TIntDoubleHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TIntDoubleHashMap clonePar(int numProcs) {
        TIntDoubleHashMap result;
        try {
            result = (TIntDoubleHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public double compute (int key,
    			BiFunction<? super Integer, ? super Double, ? extends Double> remappingFunction)	{
    	int index = index (key);
    	Double value = null;
    	if (index > 0)	value = remappingFunction.apply((Integer)key, (Double)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public double comptueIfAbsent (int key, Function<? super Integer,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public double computeIfPresent (int key,
    				BiFunction<? super Integer, ? super Double, ? extends Double> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TIntDoubleMap that;
    	int min, max;
    	Equals1 (TIntDoubleMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TIntDoubleMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Integer, ? super Double> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Integer, ? super Double> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Integer, ? super Double> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TIntDoubleHashMapSpliterator {
        final TIntDoubleHashMap map;
        ...
        TIntDoubleHashMapSpliterator(TIntDoubleHashMap m, int origin,
                           int fence, int est, int no_entry_key, double no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TIntDoubleKeySpliterator
        extends TIntDoubleHashMapSpliterator
        implements TIntSpliterator {
        TIntDoubleKeySpliterator(TIntDoubleHashMap m, int origin, int fence, int est, int no_entry, double no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TIntDoubleKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TIntDoubleKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TIntDoubleValueSpliterator
        extends TIntDoubleHashMapSpliterator
        implements TDoubleSpliterator {
        TIntDoubleValueSpliterator(TIntDoubleHashMap m, int origin, int fence, int est,
                         int no_entry_key, double no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TIntDoubleValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TIntDoubleValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TIntSet {
        ...
        public TIntSpliterator spliterator() {
        	return new TIntDoubleKeySpliterator(TIntDoubleHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public int[] toArrayPar (int numProcs) {
            return TIntDoubleHashMap.this.keysPar(numProcs);
        }
        ...
        public int[] toArrayPar (int[] dest, int numProcs)	{
        	return TIntDoubleHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntDoubleHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntDoubleHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TIntDoubleHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TIntDoubleHashMap.this.index((Integer)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TIntDoubleHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Integer> action)	{
        	TIntDoubleHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TIntDoubleHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public double merge (int key, double value, BiFunction<? super Integer, ? super Double,
    		? extends Double> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public double putPar (int key, double value) {
    ...
    private double doPut (int key, double value, int index) {
      ...
      * with key
      */
     public double putParAll (int key, double value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private double doPutAll(int key, double value, int index) {
        double previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Integer, ? extends Double> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Integer, ? extends Double> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	int[] keys;
     	double[] values;
     	PutAll2 (int a, int b, int[] c, double[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TIntDoubleMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (int[] keys, double[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(int[] keys, double[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Integer, ? super Double, ? extends Double> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Integer, ? super Double,
			? extends Double> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Integer, ? super Double,
    ...
    protected class TValueView implements TDoubleCollection {
        ...
        public TDoubleSpliterator spliterator() {
        	return new TIntDoubleValueSpliterator(TIntDoubleHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(double entry, int numProcs) {
            return TIntDoubleHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public double[] toArray() {
            return TIntDoubleHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public double[] toArrayPar(int numProcs) {
            return TIntDoubleHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public double[] toArray(double[] dest) {
            return TIntDoubleHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public double[] toArrayPar(double[] dest, int numProcs) {
            return TIntDoubleHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	double[] values = TIntDoubleHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Double> action)	{
        	TIntDoubleHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Double> action, int numProcs)	{
        	TIntDoubleHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	double[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, int[] oldK, double[] oldV, byte[] oldS){
