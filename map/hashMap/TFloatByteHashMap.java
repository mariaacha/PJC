...
public class TFloatByteHashMap extends TFloatByteHash implements TFloatByteMap, Serializable, Cloneable {
     ...
     */
    public TFloatByteHashMap (float[] keys, byte[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TFloatByteHashMap(TFloatByteMap map, int numProcs) {
        super(map.size());
        if (map instanceof TFloatByteHashMap) {
            TFloatByteHashMap hashmap = (TFloatByteHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TFloatByteHashMap clone() {
        TFloatByteHashMap result;
        try {
            result = (TFloatByteHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TFloatByteHashMap clonePar(int numProcs) {
        TFloatByteHashMap result;
        try {
            result = (TFloatByteHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public byte compute (float key,
    	...
    	int index = index (key);
    	Byte value = null;
    	if (index > 0)	value = remappingFunction.apply((Float)key, (Byte)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public byte comptueIfAbsent (float key, Function<? super Float,
    ...
    private class Equals1 implements Runnable	{
    	TFloatByteMap that;
    	int min, max;
    	Equals1 (TFloatByteMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TFloatByteMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Float, ? super Byte> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Float, ? super Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Float, ? super Byte> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TFloatByteHashMapSpliterator {
        final TFloatByteHashMap map;
        ...
        TFloatByteHashMapSpliterator(TFloatByteHashMap m, int origin,
                           int fence, int est, float no_entry_key, byte no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TFloatByteKeySpliterator
        extends TFloatByteHashMapSpliterator
        implements TFloatSpliterator {
        TFloatByteKeySpliterator(TFloatByteHashMap m, int origin, int fence, int est, float no_entry, byte no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TFloatByteKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TFloatByteKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TFloatByteValueSpliterator
        extends TFloatByteHashMapSpliterator
        implements TByteSpliterator {
        TFloatByteValueSpliterator(TFloatByteHashMap m, int origin, int fence, int est,
                         float no_entry_key, byte no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TFloatByteValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TFloatByteValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TFloatSet {
        ...
        public TFloatSpliterator spliterator() {
        	return new TFloatByteKeySpliterator(TFloatByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public float[] toArrayPar (int numProcs) {
            return TFloatByteHashMap.this.keysPar(numProcs);
        }
        ...
        public float[] toArrayPar (float[] dest, int numProcs)	{
        	return TFloatByteHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatByteHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatByteHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TFloatByteHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TFloatByteHashMap.this.index((Float)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TFloatByteHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Float> action)	{
        	TFloatByteHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TFloatByteHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public byte merge (float key, byte value, BiFunction<? super Float, ? super Byte,
    		? extends Byte> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public byte putPar (float key, byte value) {
    ...
    private byte doPut (float key, byte value, int index) {
      ...
      * with key
      */
     public byte putParAll (float key, byte value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private byte doPutAll(float key, byte value, int index) {
        byte previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Float, ? extends Byte> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Float, ? extends Byte> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	float[] keys;
     	byte[] values;
     	PutAll2 (int a, int b, float[] c, byte[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TFloatByteMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (float[] keys, byte[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(float[] keys, byte[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Float, ? super Byte, ? extends Byte> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Float, ? super Byte,
			? extends Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Float, ? super Byte,
    ...
    protected class TValueView implements TByteCollection {
        ...
        public TByteSpliterator spliterator() {
        	return new TFloatByteValueSpliterator(TFloatByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(byte entry, int numProcs) {
            return TFloatByteHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public byte[] toArray() {
            return TFloatByteHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public byte[] toArrayPar(int numProcs) {
            return TFloatByteHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public byte[] toArray(byte[] dest) {
            return TFloatByteHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public byte[] toArrayPar(byte[] dest, int numProcs) {
            return TFloatByteHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	byte[] values = TFloatByteHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Byte> action)	{
        	TFloatByteHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TFloatByteHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	byte[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, float[] oldK, byte[] oldV, byte[] oldS){
