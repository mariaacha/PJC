...
//import paralel.collection.TShortCollection;
//import paralel.collection.TObjectCollection;
//import paralel.iterator.TShortIterator;
//import paralel.spliterator.TShortSpliterator;
...
import PJC.hash.TShortShortHash;
import PJC.iterator.TShortIterator;
import PJC.list.ArraysPar;
import PJC.map.TShortShortMap;
import PJC.map.hashMap.HashFunctions;
import PJC.map.iterator.TShortShortIterator;
import PJC.set.TShortSet;
import PJC.spliterator.TShortSpliterator;

public class TShortShortHashMap extends TShortShortHash implements TShortShortMap, Serializable, Cloneable {
     ...
     */
    public TShortShortHashMap (short[] keys, short[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TShortShortHashMap(TShortShortMap map, int numProcs) {
        super(map.size());
        if (map instanceof TShortShortHashMap) {
            TShortShortHashMap hashmap = (TShortShortHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TShortShortHashMap clone() {
        TShortShortHashMap result;
        try {
            result = (TShortShortHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TShortShortHashMap clonePar(int numProcs) {
        TShortShortHashMap result;
        try {
            result = (TShortShortHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public short compute (short key,
    	...
    	int index = index (key);
    	Short value = null;
    	if (index > 0)	value = remappingFunction.apply((Short)key, (Short)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public short comptueIfAbsent (short key, Function<? super Short,
    ...
    private class Equals1 implements Runnable	{
    	TShortShortMap that;
    	int min, max;
    	Equals1 (TShortShortMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TShortShortMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Short, ? super Short> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Short, ? super Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Short, ? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TShortShortHashMapSpliterator {
        final TShortShortHashMap map;
        ...
        TShortShortHashMapSpliterator(TShortShortHashMap m, int origin,
                           int fence, int est, short no_entry_key, short no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TShortShortKeySpliterator
        extends TShortShortHashMapSpliterator
        implements TShortSpliterator {
        TShortShortKeySpliterator(TShortShortHashMap m, int origin, int fence, int est, short no_entry, short no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TShortShortKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TShortShortKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TShortShortValueSpliterator
        extends TShortShortHashMapSpliterator
        implements TShortSpliterator {
        TShortShortValueSpliterator(TShortShortHashMap m, int origin, int fence, int est,
                         short no_entry_key, short no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TShortShortValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TShortShortValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TShortSet {
        ...
        public TShortSpliterator spliterator() {
        	return new TShortShortKeySpliterator(TShortShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public short[] toArrayPar (int numProcs) {
            return TShortShortHashMap.this.keysPar(numProcs);
        }
        ...
        public short[] toArrayPar (short[] dest, int numProcs)	{
        	return TShortShortHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortShortHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortShortHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TShortShortHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TShortShortHashMap.this.index((Short)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TShortShortHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Short> action)	{
        	TShortShortHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TShortShortHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public short merge (short key, short value, BiFunction<? super Short, ? super Short,
    		? extends Short> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public short putPar (short key, short value) {
    ...
    private short doPut (short key, short value, int index) {
      ...
      * with key
      */
     public short putParAll (short key, short value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private short doPutAll(short key, short value, int index) {
        short previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Short, ? extends Short> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Short, ? extends Short> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	short[] keys;
     	short[] values;
     	PutAll2 (int a, int b, short[] c, short[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TShortShortMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (short[] keys, short[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(short[] keys, short[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Short, ? super Short, ? extends Short> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Short, ? super Short,
			? extends Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Short, ? super Short,
    ...
    protected class TValueView implements TShortCollection {
        ...
        /** {@inheritDoc} */
        public TShortSpliterator spliterator() {
        	return new TShortShortValueSpliterator(TShortShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        /** {@inheritDoc} */
        public boolean containsPar(short entry, int numProcs) {
            return TShortShortHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public short[] toArray() {
            return TShortShortHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public short[] toArrayPar(int numProcs) {
            return TShortShortHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public short[] toArray(short[] dest) {
            return TShortShortHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public short[] toArrayPar(short[] dest, int numProcs) {
            return TShortShortHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	short[] values = TShortShortHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        /** {@inheritDoc} */
        public void forEach (Consumer<? super Short> action)	{
        	TShortShortHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TShortShortHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	short[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, short[] oldK, short[] oldV, byte[] oldS){
