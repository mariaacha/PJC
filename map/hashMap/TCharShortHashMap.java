...
public class TCharShortHashMap extends TCharShortHash implements TCharShortMap, Serializable, Cloneable {
     ...
     */
    public TCharShortHashMap (char[] keys, short[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TCharShortHashMap(TCharShortMap map, int numProcs) {
        super(map.size());
        if (map instanceof TCharShortHashMap) {
            TCharShortHashMap hashmap = (TCharShortHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TCharShortHashMap clone() {
        TCharShortHashMap result;
        try {
            result = (TCharShortHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TCharShortHashMap clonePar(int numProcs) {
        TCharShortHashMap result;
        try {
            result = (TCharShortHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public short compute (char key,
    	...
    	int index = index (key);
    	Short value = null;
    	if (index > 0)	value = remappingFunction.apply((Character)key, (Short)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public short comptueIfAbsent (char key, Function<? super Character,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public short computeIfPresent (char key,
    				BiFunction<? super Character, ? super Short, ? extends Short> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TCharShortMap that;
    	int min, max;
    	Equals1 (TCharShortMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TCharShortMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Character, ? super Short> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Character, ? super Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Character, ? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TCharShortHashMapSpliterator {
        final TCharShortHashMap map;
        ...
        TCharShortHashMapSpliterator(TCharShortHashMap m, int origin,
                           int fence, int est, char no_entry_key, short no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TCharShortKeySpliterator
        extends TCharShortHashMapSpliterator
        implements TCharSpliterator {
        TCharShortKeySpliterator(TCharShortHashMap m, int origin, int fence, int est, char no_entry, short no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TCharShortKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TCharShortKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TCharShortValueSpliterator
        extends TCharShortHashMapSpliterator
        implements TShortSpliterator {
        TCharShortValueSpliterator(TCharShortHashMap m, int origin, int fence, int est,
                         char no_entry_key, short no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TCharShortValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TCharShortValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TCharSet {
        ...
        public TCharSpliterator spliterator() {
        	return new TCharShortKeySpliterator(TCharShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public char[] toArrayPar (int numProcs) {
            return TCharShortHashMap.this.keysPar(numProcs);
        }
        ...
        public char[] toArrayPar (char[] dest, int numProcs)	{
        	return TCharShortHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharShortHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharShortHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TCharShortHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TCharShortHashMap.this.index((Character)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TCharShortHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Character> action)	{
        	TCharShortHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TCharShortHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public short merge (char key, short value, BiFunction<? super Character, ? super Short,
    		? extends Short> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public short putPar (char key, short value) {
    ...
    private short doPut (char key, short value, int index) {
      ...
      * with key
      */
     public short putParAll (char key, short value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private short doPutAll(char key, short value, int index) {
        short previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Character, ? extends Short> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Character, ? extends Short> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	char[] keys;
     	short[] values;
     	PutAll2 (int a, int b, char[] c, short[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TCharShortMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (char[] keys, short[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(char[] keys, short[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Character, ? super Short, ? extends Short> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Character, ? super Short,
			? extends Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Character, ? super Short,
    ...
    protected class TValueView implements TShortCollection {
        ...
        public TShortSpliterator spliterator() {
        	return new TCharShortValueSpliterator(TCharShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(short entry, int numProcs) {
            return TCharShortHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public short[] toArray() {
            return TCharShortHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public short[] toArrayPar(int numProcs) {
            return TCharShortHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public short[] toArray(short[] dest) {
            return TCharShortHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public short[] toArrayPar(short[] dest, int numProcs) {
            return TCharShortHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	short[] values = TCharShortHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Short> action)	{
        	TCharShortHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TCharShortHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	short[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, char[] oldK, short[] oldV, byte[] oldS){
