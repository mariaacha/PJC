...
public class TIntObjectHashMap extends TIntObjectHash implements TIntObjectMap, Serializable, Cloneable {
     ...
     */
    public TIntObjectHashMap (int initialCapacity, float loadFactor,
        int noEntryKey, Object noEntryValue) {
        super(initialCapacity, loadFactor, noEntryKey, noEntryValue);
    }


    /**
     * Creates a new <code>TIntObjectHashMap</code> instance containing
     ...
     */
    public TIntObjectHashMap (int[] keys, Object[] values) {
     ...
     */
    public TIntObjectHashMap (int[] keys, Object[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * all of the entries in the map passed in.
     *
     * @param map a <tt>TIntObjectMap</tt> that will be duplicated.
     */
    public TIntObjectHashMap (TIntObjectMap map) {
        super(map.size());
        if (map instanceof TIntObjectHashMap) {
            TIntObjectHashMap hashmap = (TIntObjectHashMap) map;
     ...
     * @param numProcs number of processors that will be used
     */
    public TIntObjectHashMap(TIntObjectMap map, int numProcs) {
        super(map.size());
        if (map instanceof TIntObjectHashMap) {
            TIntObjectHashMap hashmap = (TIntObjectHashMap) map;
            this._loadFactor = hashmap._loadFactor;
    ...
    public TIntObjectHashMap clone() {
        TIntObjectHashMap result;
        try {
            result = (TIntObjectHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TIntObjectHashMap clonePar(int numProcs) {
        TIntObjectHashMap result;
        try {
            result = (TIntObjectHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public Object compute (int key,
    			BiFunction<? super Integer, ? super Object, ? extends Object> remappingFunction)	{
    	int index = index (key);
    	Object value = null;
    	if (index > 0)	value = remappingFunction.apply((Integer)key, (Object)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public Object comptueIfAbsent (int key, Function<? super Integer,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public Object computeIfPresent (int key,
    				BiFunction<? super Integer, ? super Object, ? extends Object> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TIntObjectMap that;
    	int min, max;
    	Equals1 (TIntObjectMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TIntObjectMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Integer, ? super Object> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Integer, ? super Object> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Integer, ? super Object> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TIntObjectHashMapSpliterator {
        ...
        TIntObjectHashMapSpliterator(TIntObjectHashMap m, int origin,
                           int fence, int est, int no_entry_key, Object no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TIntObjectKeySpliterator
        extends TIntObjectHashMapSpliterator
        implements TIntSpliterator {
        TIntObjectKeySpliterator(TIntObjectHashMap m, int origin, int fence, int est, int no_entry, Object no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TIntObjectKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TIntObjectKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TIntObjectValueSpliterator
        extends TIntObjectHashMapSpliterator
        implements TObjectSpliterator {
        TIntObjectValueSpliterator(TIntObjectHashMap m, int origin, int fence, int est,
                         int no_entry_key, Object no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TIntObjectValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TIntObjectValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TIntSet {
        ...
        public TIntIterator iterator() {
            return new TIntObjectKeyHashIterator (TIntObjectHashMap.this);
        }
        
        /** {@inheritDoc} */
        public TIntSpliterator spliterator() {
        	return new TIntObjectKeySpliterator(TIntObjectHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean contains (int entry) {
            return TIntObjectHashMap.this.contains(entry);
        }
        ...
        public int[] toArrayPar (int numProcs) {
            return TIntObjectHashMap.this.keysPar(numProcs);
        }
        ...
        public int[] toArrayPar (int[] dest, int numProcs)	{
        	return TIntObjectHashMap.this.keysPar(dest, numProcs);
        }


        /**
         * Unsupported when operating upon a Key Set view of a TIntObjectMap
         * <p/>
         * {@inheritDoc}
         */
        ...
        public boolean remove (int entry) {
            return no_entry_value != TIntObjectHashMap.this.remove(entry);
        }
        /** {@inheritDoc} */
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
                    ...
                    if (array[i] instanceof Integer) {
                        ...
                        if (!TIntObjectHashMap.this.containsKey(ele)) {
                            boolGlobal = false;
                            return;
                        }
                    } 
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntObjectHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntObjectHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (int[] array) {
            for (int element : array) {
                if (!TIntObjectHashMap.this.contains(element)) {
                    return false;
                }
            }
            return true;
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TIntObjectHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TIntObjectHashMap.this.index((Integer)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TIntObjectHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Integer> action)	{
        	TIntObjectHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TIntObjectHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public Object merge (int key, Object value, BiFunction<? super Integer, ? super Object,
    		? extends Object> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public Object putPar (int key, Object value) {
     ...
     */
    private Object doPut (int key, Object value, int index) {
      ...
      * with key
      */
     public Object putParAll (int key, Object value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private Object doPutAll(int key, Object value, int index) {
        Object previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Integer, ? extends Object> map) {
        ...
        for (Map.Entry<? extends Integer, ? extends Object> entry : map.entrySet()) {
            //this.put(entry.getKey().intValue(), entry.getValue().ObjectValue());
        	this.put(entry.getKey().intValue(), entry.getValue());
        }
    }
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Integer, ? extends Object> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	int[] keys;
     	Object[] values;
     	PutAll2 (int a, int b, int[] c, Object[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TIntObjectMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (int[] keys, Object[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(int[] keys, Object[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    /** {@inheritDoc} */
    public Object replace (int key, Object value)	{
    	int index = index (key);
    	if (index > 0)	{
    ...
    /** {@inheritDoc} */
    public boolean replace (int key, Object oldValue, Object newValue)	{
    	int index = index (key);
    ...
    public void replaceAll (BiFunction 
    			<? super Integer, ? super Object, ? extends Object> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Integer, ? super Object,
			? extends Object> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Integer, ? super Object,
    ...
    protected class TValueView implements TObjectCollection {
        ...
        public TObjectSpliterator spliterator() {
        	return new TIntObjectValueSpliterator(TIntObjectHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean contains (Object entry) {
            return TIntObjectHashMap.this.containsValue(entry);
        }
        @Override
        /** {@inheritDoc} */
        public boolean containsPar(Object entry, int numProcs) {
            return TIntObjectHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public Object[] toArray() {
            return TIntObjectHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public Object[] toArrayPar(int numProcs) {
            return TIntObjectHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public Object[] toArray(Object[] dest) {
            return TIntObjectHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public Object[] toArrayPar(Object[] dest, int numProcs) {
            return TIntObjectHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	Object[] values = TIntObjectHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public boolean containsAll(Collection<?> collection) {
            for (Object element : collection) {
                if (element instanceof Object) {
                    ...
                    if (!TIntObjectHashMap.this.containsValue(element)) {
                        return false;
                    }
                } else {
                    return false;
        ...
        public boolean containsAll (TObjectCollection collection) {
            ...
            while (iter.hasNext()) {
                if (!TIntObjectHashMap.this.containsValue(iter.next())) {
                    return false;
                }
            }
            return true;
        ...
        public void forEach (Consumer<? super Object> action)	{
        	TIntObjectHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Object> action, int numProcs)	{
        	TIntObjectHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    class TIntObjectKeyHashIterator extends THashPrimitiveIterator implements TIntIterator {
         ...
         */
        TIntObjectKeyHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
        ...
        public void remove() {
            ...
            try {
                ...
                TIntObjectHashMap.this.removeAt(_index);
            }
            finally {
    ...
    private class Rehash1 implements Runnable	{
    	...
    	Object[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, int[] oldK, Object[] oldV, byte[] oldS){
    ...
    public boolean adjustValue (int key, Object amount) {
        int index = index(key);
        if (index < 0) {
            return false;
        } 
    ...
    public Object adjustOrPutValue (int key, Object adjust_amount, Object put_amount) {
        int index = insertKey(key);
    ...
    class TIntObjectValueHashIterator extends THashPrimitiveIterator implements TObjectIterator {
         ...
         * @param hash the <tt>TPrimitiveHash</tt> we will be iterating over.
         */
        TIntObjectValueHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    class TIntObjectHashIterator extends THashPrimitiveIterator implements TIntObjectIterator {
         ...
         * Creates an iterator over the specified map
         *
         * @param map the <tt>TIntObjectHashMap</tt> we will be iterating over.
         */
        TIntObjectHashIterator (TIntObjectHashMap map) {
            super(map);
        }
