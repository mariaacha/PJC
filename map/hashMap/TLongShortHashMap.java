...
public class TLongShortHashMap extends TLongShortHash implements TLongShortMap, Serializable, Cloneable {
     ...
     */
    public TLongShortHashMap (long[] keys, short[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TLongShortHashMap(TLongShortMap map, int numProcs) {
        super(map.size());
        if (map instanceof TLongShortHashMap) {
            TLongShortHashMap hashmap = (TLongShortHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TLongShortHashMap clone() {
        TLongShortHashMap result;
        try {
            result = (TLongShortHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TLongShortHashMap clonePar(int numProcs) {
        TLongShortHashMap result;
        try {
            result = (TLongShortHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public short compute (long key,
    	...
    	int index = index (key);
    	Short value = null;
    	if (index > 0)	value = remappingFunction.apply((Long)key, (Short)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public short comptueIfAbsent (long key, Function<? super Long,
    ...
    private class Equals1 implements Runnable	{
    	TLongShortMap that;
    	int min, max;
    	Equals1 (TLongShortMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TLongShortMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Long, ? super Short> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Long, ? super Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Long, ? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TLongShortHashMapSpliterator {
        final TLongShortHashMap map;
        ...
        TLongShortHashMapSpliterator(TLongShortHashMap m, int origin,
                           int fence, int est, long no_entry_key, short no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TLongShortKeySpliterator
        extends TLongShortHashMapSpliterator
        implements TLongSpliterator {
        TLongShortKeySpliterator(TLongShortHashMap m, int origin, int fence, int est, long no_entry, short no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TLongShortKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TLongShortKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TLongShortValueSpliterator
        extends TLongShortHashMapSpliterator
        implements TShortSpliterator {
        TLongShortValueSpliterator(TLongShortHashMap m, int origin, int fence, int est,
                         long no_entry_key, short no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TLongShortValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TLongShortValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TLongSet {
        ...
        public TLongSpliterator spliterator() {
        	return new TLongShortKeySpliterator(TLongShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public long[] toArrayPar (int numProcs) {
            return TLongShortHashMap.this.keysPar(numProcs);
        }
        ...
        public long[] toArrayPar (long[] dest, int numProcs)	{
        	return TLongShortHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TLongShortHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TLongShortHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TLongShortHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TLongShortHashMap.this.index((Long)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TLongShortHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Long> action)	{
        	TLongShortHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Long> action, int numProcs)	{
        	TLongShortHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public short merge (long key, short value, BiFunction<? super Long, ? super Short,
    		? extends Short> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public short putPar (long key, short value) {
    ...
    private short doPut (long key, short value, int index) {
      ...
      * with key
      */
     public short putParAll (long key, short value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private short doPutAll(long key, short value, int index) {
        short previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Long, ? extends Short> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Long, ? extends Short> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	long[] keys;
     	short[] values;
     	PutAll2 (int a, int b, long[] c, short[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TLongShortMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (long[] keys, short[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(long[] keys, short[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Long, ? super Short, ? extends Short> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Long, ? super Short,
			? extends Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Long, ? super Short,
    ...
    protected class TValueView implements TShortCollection {
        ...
        public TShortSpliterator spliterator() {
        	return new TLongShortValueSpliterator(TLongShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(short entry, int numProcs) {
            return TLongShortHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public short[] toArray() {
            return TLongShortHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public short[] toArrayPar(int numProcs) {
            return TLongShortHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public short[] toArray(short[] dest) {
            return TLongShortHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public short[] toArrayPar(short[] dest, int numProcs) {
            return TLongShortHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	short[] values = TLongShortHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Short> action)	{
        	TLongShortHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TLongShortHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	short[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, long[] oldK, short[] oldV, byte[] oldS){
