...
public class TCharIntHashMap extends TCharIntHash implements TCharIntMap, Serializable, Cloneable {
     ...
     *
     * @param initialCapacity an <code>float</code> value
     ...
     */
    public TCharIntHashMap (char[] keys, int[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TCharIntHashMap(TCharIntMap map, int numProcs) {
        super(map.size());
        if (map instanceof TCharIntHashMap) {
            TCharIntHashMap hashmap = (TCharIntHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TCharIntHashMap clone() {
        TCharIntHashMap result;
        try {
            result = (TCharIntHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TCharIntHashMap clonePar(int numProcs) {
        TCharIntHashMap result;
        try {
            result = (TCharIntHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public int compute (char key,
    			BiFunction<? super Character, ? super Integer, ? extends Integer> remappingFunction)	{
    	int index = index (key);
    	Integer value = null;
    	if (index > 0)	value = remappingFunction.apply((Character)key, (Integer)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public int comptueIfAbsent (char key, Function<? super Character,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public int computeIfPresent (char key,
    				BiFunction<? super Character, ? super Integer, ? extends Integer> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TCharIntMap that;
    	int min, max;
    	Equals1 (TCharIntMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TCharIntMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Character, ? super Integer> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Character, ? super Integer> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Character, ? super Integer> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TCharIntHashMapSpliterator {
        final TCharIntHashMap map;
        ...
        TCharIntHashMapSpliterator(TCharIntHashMap m, int origin,
                           int fence, int est, char no_entry_key, int no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TCharIntKeySpliterator
        extends TCharIntHashMapSpliterator
        implements TCharSpliterator {
        TCharIntKeySpliterator(TCharIntHashMap m, int origin, int fence, int est, char no_entry, int no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TCharIntKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TCharIntKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TCharIntValueSpliterator
        extends TCharIntHashMapSpliterator
        implements TIntSpliterator {
        TCharIntValueSpliterator(TCharIntHashMap m, int origin, int fence, int est,
                         char no_entry_key, int no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TCharIntValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TCharIntValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TCharSet {
        ...
        public TCharSpliterator spliterator() {
        	return new TCharIntKeySpliterator(TCharIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public char[] toArrayPar (int numProcs) {
            return TCharIntHashMap.this.keysPar(numProcs);
        }
        ...
        public char[] toArrayPar (char[] dest, int numProcs)	{
        	return TCharIntHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharIntHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharIntHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TCharIntHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TCharIntHashMap.this.index((Character)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TCharIntHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Character> action)	{
        	TCharIntHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TCharIntHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public int merge (char key, int value, BiFunction<? super Character, ? super Integer,
    		? extends Integer> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public int putPar (char key, int value) {
    ...
    private int doPut (char key, int value, int index) {
      ...
      * with key
      */
     public int putParAll (char key, int value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private int doPutAll(char key, int value, int index) {
        int previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Character, ? extends Integer> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Character, ? extends Integer> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	char[] keys;
     	int[] values;
     	PutAll2 (int a, int b, char[] c, int[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TCharIntMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (char[] keys, int[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(char[] keys, int[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Character, ? super Integer, ? extends Integer> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Character, ? super Integer,
			? extends Integer> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Character, ? super Integer,
    ...
    protected class TValueView implements TIntCollection {
        ...
        public TIntSpliterator spliterator() {
        	return new TCharIntValueSpliterator(TCharIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(int entry, int numProcs) {
            return TCharIntHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public int[] toArray() {
            return TCharIntHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public int[] toArrayPar(int numProcs) {
            return TCharIntHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public int[] toArray(int[] dest) {
            return TCharIntHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public int[] toArrayPar(int[] dest, int numProcs) {
            return TCharIntHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int[] values = TCharIntHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Integer> action)	{
        	TCharIntHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TCharIntHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	int[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, char[] oldK, int[] oldV, byte[] oldS){
