...
public class TFloatCharHashMap extends TFloatCharHash implements TFloatCharMap, Serializable, Cloneable {
     ...
     */
    public TFloatCharHashMap (float[] keys, char[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TFloatCharHashMap(TFloatCharMap map, int numProcs) {
        super(map.size());
        if (map instanceof TFloatCharHashMap) {
            TFloatCharHashMap hashmap = (TFloatCharHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TFloatCharHashMap clone() {
        TFloatCharHashMap result;
        try {
            result = (TFloatCharHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TFloatCharHashMap clonePar(int numProcs) {
        TFloatCharHashMap result;
        try {
            result = (TFloatCharHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public char compute (float key,
    			BiFunction<? super Float, ? super Character, ? extends Character> remappingFunction)	{
    	int index = index (key);
    	Character value = null;
    	if (index > 0)	value = remappingFunction.apply((Float)key, (Character)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public char comptueIfAbsent (float key, Function<? super Float,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public char computeIfPresent (float key,
    				BiFunction<? super Float, ? super Character, ? extends Character> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TFloatCharMap that;
    	int min, max;
    	Equals1 (TFloatCharMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TFloatCharMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Float, ? super Character> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Float, ? super Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Float, ? super Character> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TFloatCharHashMapSpliterator {
        final TFloatCharHashMap map;
        ...
        TFloatCharHashMapSpliterator(TFloatCharHashMap m, int origin,
                           int fence, int est, float no_entry_key, char no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TFloatCharKeySpliterator
        extends TFloatCharHashMapSpliterator
        implements TFloatSpliterator {
        TFloatCharKeySpliterator(TFloatCharHashMap m, int origin, int fence, int est, float no_entry, char no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TFloatCharKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TFloatCharKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TFloatCharValueSpliterator
        extends TFloatCharHashMapSpliterator
        implements TCharSpliterator {
        TFloatCharValueSpliterator(TFloatCharHashMap m, int origin, int fence, int est,
                         float no_entry_key, char no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TFloatCharValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TFloatCharValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TFloatSet {
        ...
        public TFloatSpliterator spliterator() {
        	return new TFloatCharKeySpliterator(TFloatCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public float[] toArrayPar (int numProcs) {
            return TFloatCharHashMap.this.keysPar(numProcs);
        }
        ...
        public float[] toArrayPar (float[] dest, int numProcs)	{
        	return TFloatCharHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatCharHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatCharHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TFloatCharHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TFloatCharHashMap.this.index((Float)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TFloatCharHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Float> action)	{
        	TFloatCharHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TFloatCharHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public char merge (float key, char value, BiFunction<? super Float, ? super Character,
    		? extends Character> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public char putPar (float key, char value) {
    ...
    private char doPut (float key, char value, int index) {
      ...
      * with key
      */
     public char putParAll (float key, char value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private char doPutAll(float key, char value, int index) {
        char previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Float, ? extends Character> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Float, ? extends Character> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	float[] keys;
     	char[] values;
     	PutAll2 (int a, int b, float[] c, char[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TFloatCharMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (float[] keys, char[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(float[] keys, char[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Float, ? super Character, ? extends Character> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Float, ? super Character,
			? extends Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Float, ? super Character,
    ...
    protected class TValueView implements TCharCollection {
        ...
        public TCharSpliterator spliterator() {
        	return new TFloatCharValueSpliterator(TFloatCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(char entry, int numProcs) {
            return TFloatCharHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public char[] toArray() {
            return TFloatCharHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public char[] toArrayPar(int numProcs) {
            return TFloatCharHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public char[] toArray(char[] dest) {
            return TFloatCharHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public char[] toArrayPar(char[] dest, int numProcs) {
            return TFloatCharHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	char[] values = TFloatCharHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Character> action)	{
        	TFloatCharHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TFloatCharHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	char[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, float[] oldK, char[] oldV, byte[] oldS){
