...
public class TByteCharHashMap extends TByteCharHash implements TByteCharMap, Serializable, Cloneable {
     ...
     */
    public TByteCharHashMap (byte[] keys, char[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TByteCharHashMap(TByteCharMap map, int numProcs) {
        super(map.size());
        if (map instanceof TByteCharHashMap) {
            TByteCharHashMap hashmap = (TByteCharHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TByteCharHashMap clone() {
        TByteCharHashMap result;
        try {
            result = (TByteCharHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TByteCharHashMap clonePar(int numProcs) {
        TByteCharHashMap result;
        try {
            result = (TByteCharHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public char compute (byte key,
    			BiFunction<? super Byte, ? super Character, ? extends Character> remappingFunction)	{
    	int index = index (key);
    	Character value = null;
    	if (index > 0)	value = remappingFunction.apply((Byte)key, (Character)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public char comptueIfAbsent (byte key, Function<? super Byte,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public char computeIfPresent (byte key,
    				BiFunction<? super Byte, ? super Character, ? extends Character> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TByteCharMap that;
    	int min, max;
    	Equals1 (TByteCharMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TByteCharMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Byte, ? super Character> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Byte, ? super Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Byte, ? super Character> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TByteCharHashMapSpliterator {
        final TByteCharHashMap map;
        ...
        TByteCharHashMapSpliterator(TByteCharHashMap m, int origin,
                           int fence, int est, byte no_entry_key, char no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TByteCharKeySpliterator
        extends TByteCharHashMapSpliterator
        implements TByteSpliterator {
        TByteCharKeySpliterator(TByteCharHashMap m, int origin, int fence, int est, byte no_entry, char no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TByteCharKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TByteCharKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TByteCharValueSpliterator
        extends TByteCharHashMapSpliterator
        implements TCharSpliterator {
        TByteCharValueSpliterator(TByteCharHashMap m, int origin, int fence, int est,
                         byte no_entry_key, char no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TByteCharValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TByteCharValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TByteSet {
        ...
        public TByteSpliterator spliterator() {
        	return new TByteCharKeySpliterator(TByteCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public byte[] toArrayPar (int numProcs) {
            return TByteCharHashMap.this.keysPar(numProcs);
        }
        ...
        public byte[] toArrayPar (byte[] dest, int numProcs)	{
        	return TByteCharHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteCharHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteCharHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TByteCharHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TByteCharHashMap.this.index((Byte)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TByteCharHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Byte> action)	{
        	TByteCharHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TByteCharHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public char merge (byte key, char value, BiFunction<? super Byte, ? super Character,
    		? extends Character> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public char putPar (byte key, char value) {
    ...
    private char doPut (byte key, char value, int index) {
      ...
      * with key
      */
     public char putParAll (byte key, char value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private char doPutAll(byte key, char value, int index) {
        char previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Byte, ? extends Character> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Byte, ? extends Character> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	byte[] keys;
     	char[] values;
     	PutAll2 (int a, int b, byte[] c, char[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TByteCharMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (byte[] keys, char[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(byte[] keys, char[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Byte, ? super Character, ? extends Character> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Byte, ? super Character,
			? extends Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Byte, ? super Character,
    ...
    protected class TValueView implements TCharCollection {
        ...
        public TCharSpliterator spliterator() {
        	return new TByteCharValueSpliterator(TByteCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(char entry, int numProcs) {
            return TByteCharHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public char[] toArray() {
            return TByteCharHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public char[] toArrayPar(int numProcs) {
            return TByteCharHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public char[] toArray(char[] dest) {
            return TByteCharHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public char[] toArrayPar(char[] dest, int numProcs) {
            return TByteCharHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	char[] values = TByteCharHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Character> action)	{
        	TByteCharHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TByteCharHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	char[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, byte[] oldK, char[] oldV, byte[] oldS){
