...
public class TByteLongHashMap extends TByteLongHash implements TByteLongMap, Serializable, Cloneable {
     ...
     */
    public TByteLongHashMap (byte[] keys, long[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TByteLongHashMap(TByteLongMap map, int numProcs) {
        super(map.size());
        if (map instanceof TByteLongHashMap) {
            TByteLongHashMap hashmap = (TByteLongHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TByteLongHashMap clone() {
        TByteLongHashMap result;
        try {
            result = (TByteLongHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TByteLongHashMap clonePar(int numProcs) {
        TByteLongHashMap result;
        try {
            result = (TByteLongHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public long compute (byte key,
    	...
    	int index = index (key);
    	Long value = null;
    	if (index > 0)	value = remappingFunction.apply((Byte)key, (Long)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public long comptueIfAbsent (byte key, Function<? super Byte,
    ...
    private class Equals1 implements Runnable	{
    	TByteLongMap that;
    	int min, max;
    	Equals1 (TByteLongMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TByteLongMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Byte, ? super Long> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Byte, ? super Long> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Byte, ? super Long> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TByteLongHashMapSpliterator {
        final TByteLongHashMap map;
        ...
        TByteLongHashMapSpliterator(TByteLongHashMap m, int origin,
                           int fence, int est, byte no_entry_key, long no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TByteLongKeySpliterator
        extends TByteLongHashMapSpliterator
        implements TByteSpliterator {
        TByteLongKeySpliterator(TByteLongHashMap m, int origin, int fence, int est, byte no_entry, long no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TByteLongKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TByteLongKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TByteLongValueSpliterator
        extends TByteLongHashMapSpliterator
        implements TLongSpliterator {
        TByteLongValueSpliterator(TByteLongHashMap m, int origin, int fence, int est,
                         byte no_entry_key, long no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TByteLongValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TByteLongValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TByteSet {
        ...
        public TByteSpliterator spliterator() {
        	return new TByteLongKeySpliterator(TByteLongHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public byte[] toArrayPar (int numProcs) {
            return TByteLongHashMap.this.keysPar(numProcs);
        }
        ...
        public byte[] toArrayPar (byte[] dest, int numProcs)	{
        	return TByteLongHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteLongHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteLongHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TByteLongHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TByteLongHashMap.this.index((Byte)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TByteLongHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Byte> action)	{
        	TByteLongHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TByteLongHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public long merge (byte key, long value, BiFunction<? super Byte, ? super Long,
    		? extends Long> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public long putPar (byte key, long value) {
    ...
    private long doPut (byte key, long value, int index) {
      ...
      * with key
      */
     public long putParAll (byte key, long value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private long doPutAll(byte key, long value, int index) {
        long previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Byte, ? extends Long> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Byte, ? extends Long> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	byte[] keys;
     	long[] values;
     	PutAll2 (int a, int b, byte[] c, long[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TByteLongMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (byte[] keys, long[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(byte[] keys, long[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Byte, ? super Long, ? extends Long> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Byte, ? super Long,
			? extends Long> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Byte, ? super Long,
    ...
    protected class TValueView implements TLongCollection {
        ...
        public TLongSpliterator spliterator() {
        	return new TByteLongValueSpliterator(TByteLongHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(long entry, int numProcs) {
            return TByteLongHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public long[] toArray() {
            return TByteLongHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public long[] toArrayPar(int numProcs) {
            return TByteLongHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public long[] toArray(long[] dest) {
            return TByteLongHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public long[] toArrayPar(long[] dest, int numProcs) {
            return TByteLongHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	long[] values = TByteLongHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Long> action)	{
        	TByteLongHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Long> action, int numProcs)	{
        	TByteLongHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	long[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, byte[] oldK, long[] oldV, byte[] oldS){
