...
public class TDoubleLongHashMap extends TDoubleLongHash implements TDoubleLongMap, Serializable, Cloneable {
     ...
     */
    public TDoubleLongHashMap (double[] keys, long[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TDoubleLongHashMap(TDoubleLongMap map, int numProcs) {
        super(map.size());
        if (map instanceof TDoubleLongHashMap) {
            TDoubleLongHashMap hashmap = (TDoubleLongHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TDoubleLongHashMap clone() {
        TDoubleLongHashMap result;
        try {
            result = (TDoubleLongHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TDoubleLongHashMap clonePar(int numProcs) {
        TDoubleLongHashMap result;
        try {
            result = (TDoubleLongHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public long compute (double key,
    	...
    	int index = index (key);
    	Long value = null;
    	if (index > 0)	value = remappingFunction.apply((Double)key, (Long)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public long comptueIfAbsent (double key, Function<? super Double,
    ...
    private class Equals1 implements Runnable	{
    	TDoubleLongMap that;
    	int min, max;
    	Equals1 (TDoubleLongMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TDoubleLongMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Double, ? super Long> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Double, ? super Long> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Double, ? super Long> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TDoubleLongHashMapSpliterator {
        final TDoubleLongHashMap map;
        ...
        TDoubleLongHashMapSpliterator(TDoubleLongHashMap m, int origin,
                           int fence, int est, double no_entry_key, long no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TDoubleLongKeySpliterator
        extends TDoubleLongHashMapSpliterator
        implements TDoubleSpliterator {
        TDoubleLongKeySpliterator(TDoubleLongHashMap m, int origin, int fence, int est, double no_entry, long no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TDoubleLongKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TDoubleLongKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TDoubleLongValueSpliterator
        extends TDoubleLongHashMapSpliterator
        implements TLongSpliterator {
        TDoubleLongValueSpliterator(TDoubleLongHashMap m, int origin, int fence, int est,
                         double no_entry_key, long no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TDoubleLongValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TDoubleLongValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TDoubleSet {
        ...
        public TDoubleSpliterator spliterator() {
        	return new TDoubleLongKeySpliterator(TDoubleLongHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public double[] toArrayPar (int numProcs) {
            return TDoubleLongHashMap.this.keysPar(numProcs);
        }
        ...
        public double[] toArrayPar (double[] dest, int numProcs)	{
        	return TDoubleLongHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TDoubleLongHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TDoubleLongHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TDoubleLongHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TDoubleLongHashMap.this.index((Double)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TDoubleLongHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Double> action)	{
        	TDoubleLongHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Double> action, int numProcs)	{
        	TDoubleLongHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public long merge (double key, long value, BiFunction<? super Double, ? super Long,
    		? extends Long> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public long putPar (double key, long value) {
    ...
    private long doPut (double key, long value, int index) {
      ...
      * with key
      */
     public long putParAll (double key, long value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private long doPutAll(double key, long value, int index) {
        long previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Double, ? extends Long> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Double, ? extends Long> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	double[] keys;
     	long[] values;
     	PutAll2 (int a, int b, double[] c, long[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TDoubleLongMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (double[] keys, long[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(double[] keys, long[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Double, ? super Long, ? extends Long> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Double, ? super Long,
			? extends Long> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Double, ? super Long,
    ...
    protected class TValueView implements TLongCollection {
        ...
        public TLongSpliterator spliterator() {
        	return new TDoubleLongValueSpliterator(TDoubleLongHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(long entry, int numProcs) {
            return TDoubleLongHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public long[] toArray() {
            return TDoubleLongHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public long[] toArrayPar(int numProcs) {
            return TDoubleLongHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public long[] toArray(long[] dest) {
            return TDoubleLongHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public long[] toArrayPar(long[] dest, int numProcs) {
            return TDoubleLongHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	long[] values = TDoubleLongHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Long> action)	{
        	TDoubleLongHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Long> action, int numProcs)	{
        	TDoubleLongHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	long[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, double[] oldK, long[] oldV, byte[] oldS){
