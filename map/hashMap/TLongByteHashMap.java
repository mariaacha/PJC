...
public class TLongByteHashMap extends TLongByteHash implements TLongByteMap, Serializable, Cloneable {
     ...
     */
    public TLongByteHashMap (long[] keys, byte[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TLongByteHashMap(TLongByteMap map, int numProcs) {
        super(map.size());
        if (map instanceof TLongByteHashMap) {
            TLongByteHashMap hashmap = (TLongByteHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TLongByteHashMap clone() {
        TLongByteHashMap result;
        try {
            result = (TLongByteHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TLongByteHashMap clonePar(int numProcs) {
        TLongByteHashMap result;
        try {
            result = (TLongByteHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public byte compute (long key,
    	...
    	int index = index (key);
    	Byte value = null;
    	if (index > 0)	value = remappingFunction.apply((Long)key, (Byte)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public byte comptueIfAbsent (long key, Function<? super Long,
    ...
    private class Equals1 implements Runnable	{
    	TLongByteMap that;
    	int min, max;
    	Equals1 (TLongByteMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TLongByteMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Long, ? super Byte> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Long, ? super Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Long, ? super Byte> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TLongByteHashMapSpliterator {
        final TLongByteHashMap map;
        ...
        TLongByteHashMapSpliterator(TLongByteHashMap m, int origin,
                           int fence, int est, long no_entry_key, byte no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TLongByteKeySpliterator
        extends TLongByteHashMapSpliterator
        implements TLongSpliterator {
        TLongByteKeySpliterator(TLongByteHashMap m, int origin, int fence, int est, long no_entry, byte no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TLongByteKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TLongByteKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TLongByteValueSpliterator
        extends TLongByteHashMapSpliterator
        implements TByteSpliterator {
        TLongByteValueSpliterator(TLongByteHashMap m, int origin, int fence, int est,
                         long no_entry_key, byte no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TLongByteValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TLongByteValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TLongSet {
        ...
        public TLongSpliterator spliterator() {
        	return new TLongByteKeySpliterator(TLongByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public long[] toArrayPar (int numProcs) {
            return TLongByteHashMap.this.keysPar(numProcs);
        }
        ...
        public long[] toArrayPar (long[] dest, int numProcs)	{
        	return TLongByteHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TLongByteHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TLongByteHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TLongByteHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TLongByteHashMap.this.index((Long)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TLongByteHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Long> action)	{
        	TLongByteHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Long> action, int numProcs)	{
        	TLongByteHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public byte merge (long key, byte value, BiFunction<? super Long, ? super Byte,
    		? extends Byte> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public byte putPar (long key, byte value) {
    ...
    private byte doPut (long key, byte value, int index) {
      ...
      * with key
      */
     public byte putParAll (long key, byte value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private byte doPutAll(long key, byte value, int index) {
        byte previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Long, ? extends Byte> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Long, ? extends Byte> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	long[] keys;
     	byte[] values;
     	PutAll2 (int a, int b, long[] c, byte[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TLongByteMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (long[] keys, byte[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(long[] keys, byte[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Long, ? super Byte, ? extends Byte> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Long, ? super Byte,
			? extends Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Long, ? super Byte,
    ...
    protected class TValueView implements TByteCollection {
        ...
        public TByteSpliterator spliterator() {
        	return new TLongByteValueSpliterator(TLongByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(byte entry, int numProcs) {
            return TLongByteHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public byte[] toArray() {
            return TLongByteHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public byte[] toArrayPar(int numProcs) {
            return TLongByteHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public byte[] toArray(byte[] dest) {
            return TLongByteHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public byte[] toArrayPar(byte[] dest, int numProcs) {
            return TLongByteHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	byte[] values = TLongByteHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Byte> action)	{
        	TLongByteHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TLongByteHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	byte[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, long[] oldK, byte[] oldV, byte[] oldS){
