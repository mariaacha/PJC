...
public class TObjectFloatHashMap extends TObjectFloatHash implements TObjectFloatMap, Serializable, Cloneable {
     ...
     */
    public TObjectFloatHashMap (int initialCapacity, float loadFactor,
        Object noEntryKey, float noEntryValue) {
        super(initialCapacity, loadFactor, noEntryKey, noEntryValue);
    }


    /**
     * Creates a new <code>TObjectFloatHashMap</code> instance containing
     ...
     */
    public TObjectFloatHashMap (Object[] keys, float[] values) {
     ...
     */
    public TObjectFloatHashMap (Object[] keys, float[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * all of the entries in the map passed in.
     *
     * @param map a <tt>TObjectFloatMap</tt> that will be duplicated.
     */
    public TObjectFloatHashMap (TObjectFloatMap map) {
        super(map.size());
     ...
     * @param numProcs number of processors that will be used
     */
    public TObjectFloatHashMap(TObjectFloatMap map, int numProcs) {
        super(map.size());
        if (map instanceof TObjectFloatHashMap) {
            TObjectFloatHashMap hashmap = (TObjectFloatHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TObjectFloatHashMap clone() {
        TObjectFloatHashMap result;
        try {
            result = (TObjectFloatHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TObjectFloatHashMap clonePar(int numProcs) {
        TObjectFloatHashMap result;
        try {
            result = (TObjectFloatHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public float compute (Object key,
    			BiFunction<? super Object, ? super Float, ? extends Float> remappingFunction)	{
    	int index = index (key);
    	Float value = null;
    	if (index > 0)	value = remappingFunction.apply((Object)key, (Float)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public float comptueIfAbsent (Object key, Function<? super Object,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public float computeIfPresent (Object key,
    				BiFunction<? super Object, ? super Float, ? extends Float> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TObjectFloatMap that;
    	int min, max;
    	Equals1 (TObjectFloatMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TObjectFloatMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
     ...
     */
    public void forEach(BiConsumer<? super Object, ? super Float> action) {
        //Node<K,V>[] tab;
        if (action == null)
            throw new NullPointerException();
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Object, ? super Float> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Object, ? super Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Object, ? super Float> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TObjectFloatHashMapSpliterator {
        final TObjectFloatHashMap map;
        ...
        TObjectFloatHashMapSpliterator(TObjectFloatHashMap m, int origin,
                           int fence, int est, Object no_entry_key, float no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TObjectFloatKeySpliterator
        extends TObjectFloatHashMapSpliterator
        implements TObjectSpliterator {
        TObjectFloatKeySpliterator(TObjectFloatHashMap m, int origin, int fence, int est, Object no_entry, float no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TObjectFloatKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TObjectFloatKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TObjectFloatValueSpliterator
        extends TObjectFloatHashMapSpliterator
        implements TFloatSpliterator {
        TObjectFloatValueSpliterator(TObjectFloatHashMap m, int origin, int fence, int est,
                         Object no_entry_key, float no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TObjectFloatValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TObjectFloatValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TObjectSet {
        ...
        public TObjectIterator iterator() {
            return new TObjectFloatKeyHashIterator (TObjectFloatHashMap.this);
        }
        
        /** {@inheritDoc} */
        public TObjectSpliterator spliterator() {
        	return new TObjectFloatKeySpliterator(TObjectFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean contains (Object entry) {
            return TObjectFloatHashMap.this.contains(entry);
        }
        ...
        public Object[] toArray() {
            return TObjectFloatHashMap.this.keys();
        }

        /** {@inheritDoc} */
        public Object[] toArrayPar (int numProcs) {
            return TObjectFloatHashMap.this.keysPar(numProcs);
        }

        /** {@inheritDoc} */
        public Object[] toArray (Object[] dest) {
            return TObjectFloatHashMap.this.keys(dest);
        }
        public Object[] toArrayPar (Object[] dest, int numProcs)	{
        	return TObjectFloatHashMap.this.keysPar(dest, numProcs);
        }


        /**
         * Unsupported when operating upon a Key Set view of a TObjectFloatMap
         * <p/>
         * {@inheritDoc}
         */
        ...
        public boolean remove (Object entry) {
            return no_entry_value != TObjectFloatHashMap.this.remove(entry);
        }
        /** {@inheritDoc} */
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
                    ...
                    if (array[i] instanceof Object) {
                    	...
                    	Object ele = array[i];
                        if (!TObjectFloatHashMap.this.containsKey(ele)) {
                            boolGlobal = false;
                            return;
                        }
                    } 
        ...
        public boolean containsAll (Collection<?> collection) {
            for (Object element : collection) {
                if (element instanceof Object) {
                    ...
                    if (!TObjectFloatHashMap.this.containsKey(element)) {
                        return false;
                    }
                } else {
                    return false;
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectFloatHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (TObjectCollection collection) {
            ...
            while (iter.hasNext()) {
                if (!TObjectFloatHashMap.this.containsKey(iter.next())) {
                    return false;
                }
            }
            return true;
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectFloatHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (Object[] array) {
            for (Object element : array) {
                if (!TObjectFloatHashMap.this.contains(element)) {
                    return false;
                }
            }
            return true;
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TObjectFloatHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TObjectFloatHashMap.this.index((Object)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TObjectFloatHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Object> action)	{
        	TObjectFloatHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Object> action, int numProcs)	{
        	TObjectFloatHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public float merge (Object key, float value, BiFunction<? super Object, ? super Float,
    		? extends Float> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public float putPar (Object key, float value) {
     ...
     */
    private float doPut (Object key, float value, int index) {
      ...
      * with key
      */
     public float putParAll (Object key, float value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private float doPutAll(Object key, float value, int index) {
        float previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    /** {@inheritDoc} */
    public void putAll (Map<? extends Object, ? extends Float> map) {
        ensureCapacity(map.size());
        // could optimize this for cases when map instanceof THashMap
        for (Map.Entry<? extends Object, ? extends Float> entry : map.entrySet()) {
            //this.put(entry.getKey().ObjectValue(), entry.getValue().floatValue());
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Object, ? extends Float> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	Object[] keys;
     	float[] values;
     	PutAll2 (int a, int b, Object[] c, float[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TObjectFloatMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (Object[] keys, float[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Object[] keys, float[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    /** {@inheritDoc} */
    public float putIfAbsent (Object key, float value) {
        int index = insertKey(key);
        if (index < 0)
    ...
    /** {@inheritDoc} */
    public float replace (Object key, float value)	{
    	int index = index (key);
    	if (index > 0)	{
    ...
    /** {@inheritDoc} */
    public boolean replace (Object key, float oldValue, float newValue)	{
    	int index = index (key);
    ...
    public void replaceAll (BiFunction 
    			<? super Object, ? super Float, ? extends Float> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	int min, max;
    	BiFunction <? super Object, ? super Float, ? extends Float> function;
    	ReplaceAll1 (int a, int b, BiFunction <? super Object, ? super Float,
			? extends Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Object, ? super Float,
    ...
    protected class TValueView implements TFloatCollection {
        ...
        public TFloatIterator iterator() {
            return new TObjectFloatValueHashIterator(TObjectFloatHashMap.this);
        }

        /** {@inheritDoc} */
        public TFloatSpliterator spliterator() {
        	return new TObjectFloatValueSpliterator(TObjectFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(float entry, int numProcs) {
            return TObjectFloatHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public float[] toArray() {
            return TObjectFloatHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public float[] toArrayPar(int numProcs) {
            return TObjectFloatHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public float[] toArray(float[] dest) {
            return TObjectFloatHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public float[] toArrayPar(float[] dest, int numProcs) {
            return TObjectFloatHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int numElemProc = collection.size()/numProcs;
        	Object[] array = collection.toArray (new Object[collection.size()]);
        	float[] values = TObjectFloatHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Float> action)	{
        	TObjectFloatHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TObjectFloatHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    class TObjectFloatKeyHashIterator extends THashPrimitiveIterator implements TObjectIterator {
         ...
         */
        TObjectFloatKeyHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	float[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, Object[] oldK, float[] oldV, byte[] oldS){
    ...
    /** {@inheritDoc} */
    public float adjustOrPutValue (Object key, float adjust_amount, float put_amount) {
        int index = insertKey(key);
    ...
    class TObjectFloatValueHashIterator extends THashPrimitiveIterator implements TFloatIterator {
         ...
         * @param hash the <tt>TPrimitiveHash</tt> we will be iterating over.
         */
        TObjectFloatValueHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    class TObjectFloatHashIterator extends THashPrimitiveIterator implements TObjectFloatIterator {

        /**
         * Creates an iterator over the specified map
         *
         * @param map the <tt>TObjectFloatHashMap</tt> we will be iterating over.
         */
        TObjectFloatHashIterator (TObjectFloatHashMap map) {
            super(map);
        }
