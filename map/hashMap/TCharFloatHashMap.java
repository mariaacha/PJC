...
public class TCharFloatHashMap extends TCharFloatHash implements TCharFloatMap, Serializable, Cloneable {
     ...
     */
    public TCharFloatHashMap (char[] keys, float[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TCharFloatHashMap(TCharFloatMap map, int numProcs) {
        super(map.size());
        if (map instanceof TCharFloatHashMap) {
            TCharFloatHashMap hashmap = (TCharFloatHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TCharFloatHashMap clone() {
        TCharFloatHashMap result;
        try {
            result = (TCharFloatHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TCharFloatHashMap clonePar(int numProcs) {
        TCharFloatHashMap result;
        try {
            result = (TCharFloatHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public float compute (char key,
    			BiFunction<? super Character, ? super Float, ? extends Float> remappingFunction)	{
    	int index = index (key);
    	Float value = null;
    	if (index > 0)	value = remappingFunction.apply((Character)key, (Float)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public float comptueIfAbsent (char key, Function<? super Character,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public float computeIfPresent (char key,
    				BiFunction<? super Character, ? super Float, ? extends Float> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TCharFloatMap that;
    	int min, max;
    	Equals1 (TCharFloatMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TCharFloatMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Character, ? super Float> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Character, ? super Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Character, ? super Float> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TCharFloatHashMapSpliterator {
        final TCharFloatHashMap map;
        ...
        TCharFloatHashMapSpliterator(TCharFloatHashMap m, int origin,
                           int fence, int est, char no_entry_key, float no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TCharFloatKeySpliterator
        extends TCharFloatHashMapSpliterator
        implements TCharSpliterator {
        TCharFloatKeySpliterator(TCharFloatHashMap m, int origin, int fence, int est, char no_entry, float no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TCharFloatKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TCharFloatKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TCharFloatValueSpliterator
        extends TCharFloatHashMapSpliterator
        implements TFloatSpliterator {
        TCharFloatValueSpliterator(TCharFloatHashMap m, int origin, int fence, int est,
                         char no_entry_key, float no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TCharFloatValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TCharFloatValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TCharSet {
        ...
        public TCharSpliterator spliterator() {
        	return new TCharFloatKeySpliterator(TCharFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public char[] toArrayPar (int numProcs) {
            return TCharFloatHashMap.this.keysPar(numProcs);
        }
        ...
        public char[] toArrayPar (char[] dest, int numProcs)	{
        	return TCharFloatHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharFloatHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharFloatHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TCharFloatHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TCharFloatHashMap.this.index((Character)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TCharFloatHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Character> action)	{
        	TCharFloatHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TCharFloatHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public float merge (char key, float value, BiFunction<? super Character, ? super Float,
    		? extends Float> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public float putPar (char key, float value) {
    ...
    private float doPut (char key, float value, int index) {
      ...
      * with key
      */
     public float putParAll (char key, float value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private float doPutAll(char key, float value, int index) {
        float previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Character, ? extends Float> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Character, ? extends Float> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	char[] keys;
     	float[] values;
     	PutAll2 (int a, int b, char[] c, float[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TCharFloatMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (char[] keys, float[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(char[] keys, float[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Character, ? super Float, ? extends Float> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Character, ? super Float,
			? extends Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Character, ? super Float,
    ...
    protected class TValueView implements TFloatCollection {
        ...
        public TFloatSpliterator spliterator() {
        	return new TCharFloatValueSpliterator(TCharFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(float entry, int numProcs) {
            return TCharFloatHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public float[] toArray() {
            return TCharFloatHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public float[] toArrayPar(int numProcs) {
            return TCharFloatHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public float[] toArray(float[] dest) {
            return TCharFloatHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public float[] toArrayPar(float[] dest, int numProcs) {
            return TCharFloatHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	float[] values = TCharFloatHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Float> action)	{
        	TCharFloatHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TCharFloatHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	float[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, char[] oldK, float[] oldV, byte[] oldS){
