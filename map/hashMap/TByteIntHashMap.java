...
public class TByteIntHashMap extends TByteIntHash implements TByteIntMap, Serializable, Cloneable {
     ...
     */
    public TByteIntHashMap (byte[] keys, int[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TByteIntHashMap(TByteIntMap map, int numProcs) {
        super(map.size());
        if (map instanceof TByteIntHashMap) {
            TByteIntHashMap hashmap = (TByteIntHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TByteIntHashMap clone() {
        TByteIntHashMap result;
        try {
            result = (TByteIntHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TByteIntHashMap clonePar(int numProcs) {
        TByteIntHashMap result;
        try {
            result = (TByteIntHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public int compute (byte key,
    			BiFunction<? super Byte, ? super Integer, ? extends Integer> remappingFunction)	{
    	int index = index (key);
    	Integer value = null;
    	if (index > 0)	value = remappingFunction.apply((Byte)key, (Integer)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public int computeIfPresent (byte key,
    				BiFunction<? super Byte, ? super Integer, ? extends Integer> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TByteIntMap that;
    	int min, max;
    	Equals1 (TByteIntMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TByteIntMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Byte, ? super Integer> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Byte, ? super Integer> c)	{
    ...
    static class TByteIntHashMapSpliterator {
        final TByteIntHashMap map;
        ...
        TByteIntHashMapSpliterator(TByteIntHashMap m, int origin,
                           int fence, int est, byte no_entry_key, int no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TByteIntKeySpliterator
        extends TByteIntHashMapSpliterator
        implements TByteSpliterator {
        TByteIntKeySpliterator(TByteIntHashMap m, int origin, int fence, int est, byte no_entry, int no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TByteIntKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TByteIntKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TByteIntValueSpliterator
        extends TByteIntHashMapSpliterator
        implements TIntSpliterator {
        TByteIntValueSpliterator(TByteIntHashMap m, int origin, int fence, int est,
                         byte no_entry_key, int no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TByteIntValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TByteIntValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TByteSet {
        ...
        public TByteSpliterator spliterator() {
        	return new TByteIntKeySpliterator(TByteIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public byte[] toArrayPar (int numProcs) {
            return TByteIntHashMap.this.keysPar(numProcs);
        }
        ...
        public byte[] toArrayPar (byte[] dest, int numProcs)	{
        	return TByteIntHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteIntHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteIntHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TByteIntHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TByteIntHashMap.this.index((Byte)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TByteIntHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Byte> action)	{
        	TByteIntHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TByteIntHashMap.this.forEachKeyPar (action, numProcs);
        }
     ...
     * with key
     */
     public int putPar (byte key, int value) {
    ...
    private int doPut (byte key, int value, int index) {
      ...
      * with key
      */
     public int putParAll (byte key, int value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private int doPutAll(byte key, int value, int index) {
        int previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Byte, ? extends Integer> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Byte, ? extends Integer> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	byte[] keys;
     	int[] values;
     	PutAll2 (int a, int b, byte[] c, int[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TByteIntMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
    ...
    public void replaceAll (BiFunction 
    			<? super Byte, ? super Integer, ? extends Integer> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Byte, ? super Integer,
			? extends Integer> c)	{
    ...
    protected class TValueView implements TIntCollection {
        ...
        public TIntSpliterator spliterator() {
        	return new TByteIntValueSpliterator(TByteIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(int entry, int numProcs) {
            return TByteIntHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public int[] toArray() {
            return TByteIntHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public int[] toArrayPar(int numProcs) {
            return TByteIntHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public int[] toArray(int[] dest) {
            return TByteIntHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public int[] toArrayPar(int[] dest, int numProcs) {
            return TByteIntHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int[] values = TByteIntHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Integer> action)	{
        	TByteIntHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TByteIntHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	int[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, byte[] oldK, int[] oldV, byte[] oldS){
