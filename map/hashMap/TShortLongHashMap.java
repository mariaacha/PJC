...
public class TShortLongHashMap extends TShortLongHash implements TShortLongMap, Serializable, Cloneable {
     ...
     */
    public TShortLongHashMap (short[] keys, long[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TShortLongHashMap(TShortLongMap map, int numProcs) {
        super(map.size());
        if (map instanceof TShortLongHashMap) {
            TShortLongHashMap hashmap = (TShortLongHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TShortLongHashMap clone() {
        TShortLongHashMap result;
        try {
            result = (TShortLongHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TShortLongHashMap clonePar(int numProcs) {
        TShortLongHashMap result;
        try {
            result = (TShortLongHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public long compute (short key,
    	...
    	int index = index (key);
    	Long value = null;
    	if (index > 0)	value = remappingFunction.apply((Short)key, (Long)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public long comptueIfAbsent (short key, Function<? super Short,
    ...
    private class Equals1 implements Runnable	{
    	TShortLongMap that;
    	int min, max;
    	Equals1 (TShortLongMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TShortLongMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Short, ? super Long> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Short, ? super Long> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Short, ? super Long> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TShortLongHashMapSpliterator {
        final TShortLongHashMap map;
        ...
        TShortLongHashMapSpliterator(TShortLongHashMap m, int origin,
                           int fence, int est, short no_entry_key, long no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TShortLongKeySpliterator
        extends TShortLongHashMapSpliterator
        implements TShortSpliterator {
        TShortLongKeySpliterator(TShortLongHashMap m, int origin, int fence, int est, short no_entry, long no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TShortLongKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TShortLongKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TShortLongValueSpliterator
        extends TShortLongHashMapSpliterator
        implements TLongSpliterator {
        TShortLongValueSpliterator(TShortLongHashMap m, int origin, int fence, int est,
                         short no_entry_key, long no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TShortLongValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TShortLongValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TShortSet {
        ...
        public TShortSpliterator spliterator() {
        	return new TShortLongKeySpliterator(TShortLongHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public short[] toArrayPar (int numProcs) {
            return TShortLongHashMap.this.keysPar(numProcs);
        }
        ...
        public short[] toArrayPar (short[] dest, int numProcs)	{
        	return TShortLongHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortLongHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortLongHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TShortLongHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TShortLongHashMap.this.index((Short)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TShortLongHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Short> action)	{
        	TShortLongHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TShortLongHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public long merge (short key, long value, BiFunction<? super Short, ? super Long,
    		? extends Long> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public long putPar (short key, long value) {
    ...
    private long doPut (short key, long value, int index) {
      ...
      * with key
      */
     public long putParAll (short key, long value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private long doPutAll(short key, long value, int index) {
        long previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Short, ? extends Long> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Short, ? extends Long> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	short[] keys;
     	long[] values;
     	PutAll2 (int a, int b, short[] c, long[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TShortLongMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (short[] keys, long[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(short[] keys, long[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Short, ? super Long, ? extends Long> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Short, ? super Long,
			? extends Long> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Short, ? super Long,
    ...
    protected class TValueView implements TLongCollection {
        ...
        public TLongSpliterator spliterator() {
        	return new TShortLongValueSpliterator(TShortLongHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(long entry, int numProcs) {
            return TShortLongHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public long[] toArray() {
            return TShortLongHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public long[] toArrayPar(int numProcs) {
            return TShortLongHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public long[] toArray(long[] dest) {
            return TShortLongHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public long[] toArrayPar(long[] dest, int numProcs) {
            return TShortLongHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	long[] values = TShortLongHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Long> action)	{
        	TShortLongHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Long> action, int numProcs)	{
        	TShortLongHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	long[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, short[] oldK, long[] oldV, byte[] oldS){
