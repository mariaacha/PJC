...
//import paralel.collection.TByteCollection;
//import paralel.collection.TObjectCollection;
//import paralel.iterator.TByteIterator;
//import paralel.spliterator.TByteSpliterator;
...
import PJC.hash.TByteByteHash;
import PJC.iterator.TByteIterator;
import PJC.list.ArraysPar;
import PJC.map.TByteByteMap;
import PJC.map.hashMap.HashFunctions;
import PJC.map.iterator.TByteByteIterator;
import PJC.set.TByteSet;
import PJC.spliterator.TByteSpliterator;

public class TByteByteHashMap extends TByteByteHash implements TByteByteMap, Serializable, Cloneable {
     ...
     */
    public TByteByteHashMap (byte[] keys, byte[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TByteByteHashMap(TByteByteMap map, int numProcs) {
        super(map.size());
        if (map instanceof TByteByteHashMap) {
            TByteByteHashMap hashmap = (TByteByteHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TByteByteHashMap clone() {
        TByteByteHashMap result;
        try {
            result = (TByteByteHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TByteByteHashMap clonePar(int numProcs) {
        TByteByteHashMap result;
        try {
            result = (TByteByteHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public byte compute (byte key,
    	...
    	int index = index (key);
    	Byte value = null;
    	if (index > 0)	value = remappingFunction.apply((Byte)key, (Byte)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public byte comptueIfAbsent (byte key, Function<? super Byte,
    ...
    private class Equals1 implements Runnable	{
    	TByteByteMap that;
    	int min, max;
    	Equals1 (TByteByteMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TByteByteMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Byte, ? super Byte> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Byte, ? super Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Byte, ? super Byte> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TByteByteHashMapSpliterator {
        final TByteByteHashMap map;
        ...
        TByteByteHashMapSpliterator(TByteByteHashMap m, int origin,
                           int fence, int est, byte no_entry_key, byte no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TByteByteKeySpliterator
        extends TByteByteHashMapSpliterator
        implements TByteSpliterator {
        TByteByteKeySpliterator(TByteByteHashMap m, int origin, int fence, int est, byte no_entry, byte no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TByteByteKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TByteByteKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TByteByteValueSpliterator
        extends TByteByteHashMapSpliterator
        implements TByteSpliterator {
        TByteByteValueSpliterator(TByteByteHashMap m, int origin, int fence, int est,
                         byte no_entry_key, byte no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TByteByteValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TByteByteValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TByteSet {
        ...
        public TByteSpliterator spliterator() {
        	return new TByteByteKeySpliterator(TByteByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public byte[] toArrayPar (int numProcs) {
            return TByteByteHashMap.this.keysPar(numProcs);
        }
        ...
        public byte[] toArrayPar (byte[] dest, int numProcs)	{
        	return TByteByteHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteByteHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteByteHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TByteByteHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TByteByteHashMap.this.index((Byte)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TByteByteHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Byte> action)	{
        	TByteByteHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TByteByteHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public byte merge (byte key, byte value, BiFunction<? super Byte, ? super Byte,
    		? extends Byte> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public byte putPar (byte key, byte value) {
    ...
    private byte doPut (byte key, byte value, int index) {
      ...
      * with key
      */
     public byte putParAll (byte key, byte value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private byte doPutAll(byte key, byte value, int index) {
        byte previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Byte, ? extends Byte> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Byte, ? extends Byte> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	byte[] keys;
     	byte[] values;
     	PutAll2 (int a, int b, byte[] c, byte[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TByteByteMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (byte[] keys, byte[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(byte[] keys, byte[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Byte, ? super Byte, ? extends Byte> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Byte, ? super Byte,
			? extends Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Byte, ? super Byte,
    ...
    protected class TValueView implements TByteCollection {
        ...
        /** {@inheritDoc} */
        public TByteSpliterator spliterator() {
        	return new TByteByteValueSpliterator(TByteByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        /** {@inheritDoc} */
        public boolean containsPar(byte entry, int numProcs) {
            return TByteByteHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public byte[] toArray() {
            return TByteByteHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public byte[] toArrayPar(int numProcs) {
            return TByteByteHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public byte[] toArray(byte[] dest) {
            return TByteByteHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public byte[] toArrayPar(byte[] dest, int numProcs) {
            return TByteByteHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	byte[] values = TByteByteHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        /** {@inheritDoc} */
        public void forEach (Consumer<? super Byte> action)	{
        	TByteByteHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TByteByteHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	byte[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, byte[] oldK, byte[] oldV, byte[] oldS){
