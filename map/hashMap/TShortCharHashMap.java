...
public class TShortCharHashMap extends TShortCharHash implements TShortCharMap, Serializable, Cloneable {
     ...
     */
    public TShortCharHashMap (short[] keys, char[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TShortCharHashMap(TShortCharMap map, int numProcs) {
        super(map.size());
        if (map instanceof TShortCharHashMap) {
            TShortCharHashMap hashmap = (TShortCharHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TShortCharHashMap clone() {
        TShortCharHashMap result;
        try {
            result = (TShortCharHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TShortCharHashMap clonePar(int numProcs) {
        TShortCharHashMap result;
        try {
            result = (TShortCharHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public char compute (short key,
    			BiFunction<? super Short, ? super Character, ? extends Character> remappingFunction)	{
    	int index = index (key);
    	Character value = null;
    	if (index > 0)	value = remappingFunction.apply((Short)key, (Character)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public char comptueIfAbsent (short key, Function<? super Short,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public char computeIfPresent (short key,
    				BiFunction<? super Short, ? super Character, ? extends Character> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TShortCharMap that;
    	int min, max;
    	Equals1 (TShortCharMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TShortCharMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Short, ? super Character> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Short, ? super Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Short, ? super Character> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TShortCharHashMapSpliterator {
        final TShortCharHashMap map;
        ...
        TShortCharHashMapSpliterator(TShortCharHashMap m, int origin,
                           int fence, int est, short no_entry_key, char no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TShortCharKeySpliterator
        extends TShortCharHashMapSpliterator
        implements TShortSpliterator {
        TShortCharKeySpliterator(TShortCharHashMap m, int origin, int fence, int est, short no_entry, char no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TShortCharKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TShortCharKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TShortCharValueSpliterator
        extends TShortCharHashMapSpliterator
        implements TCharSpliterator {
        TShortCharValueSpliterator(TShortCharHashMap m, int origin, int fence, int est,
                         short no_entry_key, char no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TShortCharValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TShortCharValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TShortSet {
        ...
        public TShortSpliterator spliterator() {
        	return new TShortCharKeySpliterator(TShortCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public short[] toArrayPar (int numProcs) {
            return TShortCharHashMap.this.keysPar(numProcs);
        }
        ...
        public short[] toArrayPar (short[] dest, int numProcs)	{
        	return TShortCharHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortCharHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortCharHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TShortCharHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TShortCharHashMap.this.index((Short)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TShortCharHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Short> action)	{
        	TShortCharHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TShortCharHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public char merge (short key, char value, BiFunction<? super Short, ? super Character,
    		? extends Character> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public char putPar (short key, char value) {
    ...
    private char doPut (short key, char value, int index) {
      ...
      * with key
      */
     public char putParAll (short key, char value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private char doPutAll(short key, char value, int index) {
        char previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Short, ? extends Character> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Short, ? extends Character> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	short[] keys;
     	char[] values;
     	PutAll2 (int a, int b, short[] c, char[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TShortCharMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (short[] keys, char[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(short[] keys, char[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Short, ? super Character, ? extends Character> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Short, ? super Character,
			? extends Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Short, ? super Character,
    ...
    protected class TValueView implements TCharCollection {
        ...
        public TCharSpliterator spliterator() {
        	return new TShortCharValueSpliterator(TShortCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(char entry, int numProcs) {
            return TShortCharHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public char[] toArray() {
            return TShortCharHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public char[] toArrayPar(int numProcs) {
            return TShortCharHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public char[] toArray(char[] dest) {
            return TShortCharHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public char[] toArrayPar(char[] dest, int numProcs) {
            return TShortCharHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	char[] values = TShortCharHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Character> action)	{
        	TShortCharHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TShortCharHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	char[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, short[] oldK, char[] oldV, byte[] oldS){
