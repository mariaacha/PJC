...
public class TShortByteHashMap extends TShortByteHash implements TShortByteMap, Serializable, Cloneable {
     ...
     */
    public TShortByteHashMap (short[] keys, byte[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TShortByteHashMap(TShortByteMap map, int numProcs) {
        super(map.size());
        if (map instanceof TShortByteHashMap) {
            TShortByteHashMap hashmap = (TShortByteHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TShortByteHashMap clone() {
        TShortByteHashMap result;
        try {
            result = (TShortByteHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TShortByteHashMap clonePar(int numProcs) {
        TShortByteHashMap result;
        try {
            result = (TShortByteHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public byte compute (short key,
    	...
    	int index = index (key);
    	Byte value = null;
    	if (index > 0)	value = remappingFunction.apply((Short)key, (Byte)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public byte comptueIfAbsent (short key, Function<? super Short,
    ...
    private class Equals1 implements Runnable	{
    	TShortByteMap that;
    	int min, max;
    	Equals1 (TShortByteMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TShortByteMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Short, ? super Byte> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Short, ? super Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Short, ? super Byte> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TShortByteHashMapSpliterator {
        final TShortByteHashMap map;
        ...
        TShortByteHashMapSpliterator(TShortByteHashMap m, int origin,
                           int fence, int est, short no_entry_key, byte no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TShortByteKeySpliterator
        extends TShortByteHashMapSpliterator
        implements TShortSpliterator {
        TShortByteKeySpliterator(TShortByteHashMap m, int origin, int fence, int est, short no_entry, byte no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TShortByteKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TShortByteKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TShortByteValueSpliterator
        extends TShortByteHashMapSpliterator
        implements TByteSpliterator {
        TShortByteValueSpliterator(TShortByteHashMap m, int origin, int fence, int est,
                         short no_entry_key, byte no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TShortByteValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TShortByteValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TShortSet {
        ...
        public TShortSpliterator spliterator() {
        	return new TShortByteKeySpliterator(TShortByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public short[] toArrayPar (int numProcs) {
            return TShortByteHashMap.this.keysPar(numProcs);
        }
        ...
        public short[] toArrayPar (short[] dest, int numProcs)	{
        	return TShortByteHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortByteHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortByteHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TShortByteHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TShortByteHashMap.this.index((Short)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TShortByteHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Short> action)	{
        	TShortByteHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TShortByteHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public byte merge (short key, byte value, BiFunction<? super Short, ? super Byte,
    		? extends Byte> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public byte putPar (short key, byte value) {
    ...
    private byte doPut (short key, byte value, int index) {
      ...
      * with key
      */
     public byte putParAll (short key, byte value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private byte doPutAll(short key, byte value, int index) {
        byte previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Short, ? extends Byte> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Short, ? extends Byte> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	short[] keys;
     	byte[] values;
     	PutAll2 (int a, int b, short[] c, byte[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TShortByteMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (short[] keys, byte[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(short[] keys, byte[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Short, ? super Byte, ? extends Byte> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Short, ? super Byte,
			? extends Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Short, ? super Byte,
    ...
    protected class TValueView implements TByteCollection {
        ...
        public TByteSpliterator spliterator() {
        	return new TShortByteValueSpliterator(TShortByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(byte entry, int numProcs) {
            return TShortByteHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public byte[] toArray() {
            return TShortByteHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public byte[] toArrayPar(int numProcs) {
            return TShortByteHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public byte[] toArray(byte[] dest) {
            return TShortByteHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public byte[] toArrayPar(byte[] dest, int numProcs) {
            return TShortByteHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	byte[] values = TShortByteHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Byte> action)	{
        	TShortByteHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TShortByteHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	byte[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, short[] oldK, byte[] oldV, byte[] oldS){
