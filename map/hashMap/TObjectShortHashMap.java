...
public class TObjectShortHashMap extends TObjectShortHash implements TObjectShortMap, Serializable, Cloneable {
     ...
     *
     * @param initialCapacity an <code>int</code> value
     * @param loadFactor a <code>short</code> value
     ...
     */
    public TObjectShortHashMap (int initialCapacity, float loadFactor,
        Object noEntryKey, short noEntryValue) {
        super(initialCapacity, loadFactor, noEntryKey, noEntryValue);
    }


    /**
     * Creates a new <code>TObjectShortHashMap</code> instance containing
     ...
     */
    public TObjectShortHashMap (Object[] keys, short[] values) {
     ...
     */
    public TObjectShortHashMap (Object[] keys, short[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * all of the entries in the map passed in.
     *
     * @param map a <tt>TObjectShortMap</tt> that will be duplicated.
     */
    public TObjectShortHashMap (TObjectShortMap map) {
        super(map.size());
     ...
     * @param numProcs number of processors that will be used
     */
    public TObjectShortHashMap(TObjectShortMap map, int numProcs) {
        super(map.size());
        if (map instanceof TObjectShortHashMap) {
            TObjectShortHashMap hashmap = (TObjectShortHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TObjectShortHashMap clone() {
        TObjectShortHashMap result;
        try {
            result = (TObjectShortHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TObjectShortHashMap clonePar(int numProcs) {
        TObjectShortHashMap result;
        try {
            result = (TObjectShortHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public short compute (Object key,
    			BiFunction<? super Object, ? super Short, ? extends Short> remappingFunction)	{
    	int index = index (key);
    	Short value = null;
    	if (index > 0)	value = remappingFunction.apply((Object)key, (Short)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public short comptueIfAbsent (Object key, Function<? super Object,
    		? extends Short> remappingFunction)	{
    	int index = index(key);
    	Short value = remappingFunction.apply(key);
    	if (value == null)	{
    		return no_entry_value;
    	}
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public short computeIfPresent (Object key,
    				BiFunction<? super Object, ? super Short, ? extends Short> remappingFunction)	{
    	...
    	if ((index> 0) && (_values[index] != no_entry_value))	{
    		short value = remappingFunction.apply(key, _values[index]);
    ...
    private class ContainsValue1 implements Runnable	{
    	private int min, max;
    	private short val;
    	ContainsValue1 (short value, int a, int b)	{
     ...
     */
    public boolean containsValuePar (short value, int numProcs){
    ...
    private class Equals1 implements Runnable	{
    	TObjectShortMap that;
    	int min, max;
    	Equals1 (TObjectShortMap obj, int a, int b)	{
    	...
    	public void run ()	{
    		...
    		for (int i=min; i<max; i++) {
                if (_states[i] == FULL) {
                    ...
                    if (!(that_value instanceof Short))	{
                    	boolGlobal = false;
                    	return;
                    }
                    short this_value = _values[i];
                    if ((this_value != (Short)that_value) &&
                        (this_value != this_no_entry_value) &&
                        (that_value != null)) {
                    	boolGlobal = false;
                        return;
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TObjectShortMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
     ...
     */
    public void forEach(BiConsumer<? super Object, ? super Short> action) {
        //Node<K,V>[] tab;
        if (action == null)
            throw new NullPointerException();
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Object, ? super Short> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Object, ? super Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Object, ? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TObjectShortHashMapSpliterator {
        final TObjectShortHashMap map;
        Object currentKey;				// current key
        short currentValue;			// current value
        int index;                  // current index, modified on advance/split
        int fence;                  // one past last index
        int est;                    // size estimate
        Object _no_entry_key;			// int key that represents null
        short _no_entry_value;			// int value that represents null

        TObjectShortHashMapSpliterator(TObjectShortHashMap m, int origin,
                           int fence, int est, Object no_entry_key, short no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TObjectShortKeySpliterator
        extends TObjectShortHashMapSpliterator
        implements TObjectSpliterator {
        TObjectShortKeySpliterator(TObjectShortHashMap m, int origin, int fence, int est, Object no_entry, short no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TObjectShortKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TObjectShortKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TObjectShortValueSpliterator
        extends TObjectShortHashMapSpliterator
        implements TShortSpliterator {
        TObjectShortValueSpliterator(TObjectShortHashMap m, int origin, int fence, int est,
                         Object no_entry_key, short no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TObjectShortValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TObjectShortValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TObjectSet {
        ...
        public TObjectIterator iterator() {
            return new TObjectShortKeyHashIterator (TObjectShortHashMap.this);
        }
        
        /** {@inheritDoc} */
        public TObjectSpliterator spliterator() {
        	return new TObjectShortKeySpliterator(TObjectShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean contains (Object entry) {
            return TObjectShortHashMap.this.contains(entry);
        }
        ...
        public Object[] toArray() {
            return TObjectShortHashMap.this.keys();
        }

        /** {@inheritDoc} */
        public Object[] toArrayPar (int numProcs) {
            return TObjectShortHashMap.this.keysPar(numProcs);
        }

        /** {@inheritDoc} */
        public Object[] toArray (Object[] dest) {
            return TObjectShortHashMap.this.keys(dest);
        }
        public Object[] toArrayPar (Object[] dest, int numProcs)	{
        	return TObjectShortHashMap.this.keysPar(dest, numProcs);
        }


        /**
         * Unsupported when operating upon a Key Set view of a TObjectShortMap
         * <p/>
         * {@inheritDoc}
         */
        ...
        public boolean remove (Object entry) {
            return no_entry_value != TObjectShortHashMap.this.remove(entry);
        }
        /** {@inheritDoc} */
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
                    ...
                    if (array[i] instanceof Object) {
                    	...
                    	Object ele = array[i];
                        if (!TObjectShortHashMap.this.containsKey(ele)) {
                            boolGlobal = false;
                            return;
                        }
                    } 
        ...
        public boolean containsAll (Collection<?> collection) {
            for (Object element : collection) {
                if (element instanceof Object) {
                    ...
                    if (!TObjectShortHashMap.this.containsKey(element)) {
                        return false;
                    }
                } else {
                    return false;
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectShortHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (TObjectCollection collection) {
            ...
            while (iter.hasNext()) {
                if (!TObjectShortHashMap.this.containsKey(iter.next())) {
                    return false;
                }
            }
            return true;
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectShortHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (Object[] array) {
            for (Object element : array) {
                if (!TObjectShortHashMap.this.contains(element)) {
                    return false;
                }
            }
            return true;
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TObjectShortHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TObjectShortHashMap.this.index((Object)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TObjectShortHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Object> action)	{
        	TObjectShortHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Object> action, int numProcs)	{
        	TObjectShortHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public short merge (Object key, short value, BiFunction<? super Object, ? super Short,
    		? extends Short> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
    	...
    	Short newValue = remappingFunction.apply (_set[index], value);
    	if (newValue != null)	{
    		put (key, newValue);
    		return newValue;
    	}
     ...
     * with key
     */
     public short putPar (Object key, short value) {
     ...
     */
    private short doPut (Object key, short value, int index) {
      ...
      * with key
      */
     public short putParAll (Object key, short value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private short doPutAll(Object key, short value, int index) {
        short previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    /** {@inheritDoc} */
    public void putAll (Map<? extends Object, ? extends Short> map) {
        ensureCapacity(map.size());
        // could optimize this for cases when map instanceof THashMap
        for (Map.Entry<? extends Object, ? extends Short> entry : map.entrySet()) {
            //this.put(entry.getKey().ObjectValue(), entry.getValue().shortValue());
        	this.put(entry.getKey(), entry.getValue().shortValue());
        }
    }
     	...
     	public void run ()	{
     		...
     		for (int i=min; i<max; i++)	{
     			if (entradas[i].getKey() instanceof Integer)	{
     				...
     				valor = ((Short) entradas[i].getValue()).shortValue();
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Object, ? extends Short> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	Object[] keys;
     	short[] values;
     	PutAll2 (int a, int b, Object[] c, short[] d)	{
     ...
     /** {@inheritDoc} */
     public void putAll (TObjectShortMap map) {
         ensureCapacity(map.size());
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TObjectShortMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
         Object[] set = map.keySet().toArrayPar(numProcs);
         short[] values = map.values().toArrayPar(numProcs);
         int size = set.length;
     ...
     /** {@inheritDoc} */
     public void putAll (Object[] keys, short[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Object[] keys, short[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    /** {@inheritDoc} */
    public short putIfAbsent (Object key, short value) {
        int index = insertKey(key);
        if (index < 0)
    ...
    /** {@inheritDoc} */
    public short replace (Object key, short value)	{
    	int index = index (key);
    	if (index > 0)	{
    		short oldValue = _values[index];
    		_values[index] = value;
    		return oldValue;
    	}
    	return no_entry_value;
    ...
    /** {@inheritDoc} */
    public boolean replace (Object key, short oldValue, short newValue)	{
    	int index = index (key);
    ...
    public void replaceAll (BiFunction 
    			<? super Object, ? super Short, ? extends Short> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    			short newValue = function.apply(_set[i], _values[i]);
    			_values[i] = newValue;
    		}
    	}
    }
    private class ReplaceAll1 implements Runnable	{
    	int min, max;
    	BiFunction <? super Object, ? super Short, ? extends Short> function;
    	ReplaceAll1 (int a, int b, BiFunction <? super Object, ? super Short,
			? extends Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Object, ? super Short,
    			? extends Short> function, int numProcs)	{
    	int numElemProc = _values.length/numProcs;
    	int numMin, numMax;
        threads = new Thread[numProcs];
    	for (int i=0; i<numProcs; i++)	{
    ...
    /** {@inheritDoc} */
    public TShortCollection values ()	{
    	TShortCollection c = new TValueView ();
    	return c;
    }
    ...
    protected class TValueView implements TShortCollection {
        ...
        public TShortIterator iterator() {
            return new TObjectShortValueHashIterator(TObjectShortHashMap.this);
        }

        /** {@inheritDoc} */
        public TShortSpliterator spliterator() {
        	return new TObjectShortValueSpliterator(TObjectShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean equalsPar2 (Object o, int numProcs)	{
        	boolGlobal = false;
        	if (o instanceof TShortCollection)	{
        		TShortCollection that = (TShortCollection) o;
        		if (this.size() != that.size())	{
        			return false;
        		}
        		threads = new Thread[numProcs];
        		...
        		int numElemProc = _values.length/numProcs;
        		int numMin, numMax;
        		short[] array = that.toArrayPar(numProcs);
        		for (int i=0; i<numProcs; i++)	{
        			numMin = limiteMin (i, numElemProc, 0);
            		numMax = limiteMax (i, numElemProc, 0, numProcs, _values.length);
        ...
        private class Equals3 implements Runnable	{
        	int min, max;
        	TShortCollection that;
        	Equals3 (int a, int b, TShortCollection c)	{
        		min = a; max = b; that = c;
        	}
        	public void run ()	{
        		for (int i=min; i<max; i++)	{
        ...
        public boolean containsPar(short entry, int numProcs) {
            return TObjectShortHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public short[] toArray() {
            return TObjectShortHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public short[] toArrayPar(int numProcs) {
            return TObjectShortHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public short[] toArray(short[] dest) {
            return TObjectShortHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public short[] toArrayPar(short[] dest, int numProcs) {
            return TObjectShortHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        private class ContainsAll1 implements Runnable	{
        	...
        	Object[] array;
        	short[] values;
        	ContainsAll1 (int a, int b, Object[] c, short[] d)	{
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int numElemProc = collection.size()/numProcs;
        	Object[] array = collection.toArray (new Object[collection.size()]);
        	short[] values = TObjectShortHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        private class ContainsAll2 implements Runnable	{
        	int min, max;
        	short[] array, values;
        	ContainsAll2 (int a, int b, short[] c, short[] d)	{
        		min = a; max = b; array = c; values = d;
        	}
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        ...
        private class RetainAll1 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				if (Arrays.binarySearch(array, (Short)_values[i]) < 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        			}
        ...
        private class RemoveAll2 implements Runnable	{
        	int min, max;
        	short[] array;
        	RemoveAll2 (int a, int b, short[] c)	{
        		min = a; max = b; array = c;
        	}
        	public void run ()	{
        		int removed=0;
        ...
        public void forEach (Consumer<? super Short> action)	{
        	TObjectShortHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TObjectShortHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    class TObjectShortKeyHashIterator extends THashPrimitiveIterator implements TObjectIterator {
         ...
         */
        TObjectShortKeyHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	short[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, Object[] oldK, short[] oldV, byte[] oldS){
    ...
    private class ToArrayValues1 implements Runnable	{
    	...
    	public void run()	{
    		...
    		if (idProc > 0)	{
    			...
    			System.arraycopy(shortsLocal, 0, shortsGlobal, k, j);
    		}
    		else	{
    ...
    /** {@inheritDoc} */
    protected short[] toArrayValuesPar (int numProcs)	{
    	atIntGlobal = new AtomicInteger(0);
    ...
    /** {@inheritDoc} */
    protected short[] toArrayValuesPar (short[] array, int numProcs)	{
    	if (array.length < size())
    		array = new short[size()];
    	shortsGlobal = new short[_size];
    	intsGlobal2 = new int[numProcs];
    	atIntGlobal = new AtomicInteger(0);
    	int numElemProc = _values.length/numProcs;
    	int numMin, numMax;
		...
		//array = intsGlobal;
    	System.arraycopy(shortsGlobal, 0, array, 0, atIntGlobal.get());
    ...
    /** {@inheritDoc} */
    protected short[] toArrayValues (short[] array) {
        int size = size();
        if (array.length < size) {
            array = new short[size];
        }
    ...
    private class ForEachValue1 implements Runnable	{
    	...
    	ForEachValue1 (int a, int b, Consumer<? super Short> c)	{
    		min = a; max = b; action = c;
    	}
    	public void run ()	{
    		for (int i=min; i<max; i++)	{
    ...
    /** {@inheritDoc} */
    protected void forEachValuePar (Consumer<? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0)	{
    		int numElemProc = _values.length/numProcs;
    ...
    /** {@inheritDoc} */
    protected void forEachValue (Consumer<? super Short> action)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0)	{
    		for (int i=0; i<_values.length; i++)	{
    ...
    /** {@inheritDoc} */
    public short adjustOrPutValue (Object key, short adjust_amount, short put_amount) {
        int index = insertKey(key);
    ...
    class TObjectShortValueHashIterator extends THashPrimitiveIterator implements TShortIterator {
         ...
         * @param hash the <tt>TPrimitiveHash</tt> we will be iterating over.
         */
        TObjectShortValueHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    class TObjectShortHashIterator extends THashPrimitiveIterator implements TObjectShortIterator {

        /**
         * Creates an iterator over the specified map
         *
         * @param map the <tt>TObjectShortHashMap</tt> we will be iterating over.
         */
        TObjectShortHashIterator (TObjectShortHashMap map) {
            super(map);
        }
