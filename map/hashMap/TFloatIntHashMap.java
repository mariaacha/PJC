...
public class TFloatIntHashMap extends TFloatIntHash implements TFloatIntMap, Serializable, Cloneable {
     ...
     */
    public TFloatIntHashMap (float[] keys, int[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TFloatIntHashMap(TFloatIntMap map, int numProcs) {
        super(map.size());
        if (map instanceof TFloatIntHashMap) {
            TFloatIntHashMap hashmap = (TFloatIntHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TFloatIntHashMap clone() {
        TFloatIntHashMap result;
        try {
            result = (TFloatIntHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TFloatIntHashMap clonePar(int numProcs) {
        TFloatIntHashMap result;
        try {
            result = (TFloatIntHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public int compute (float key,
    			BiFunction<? super Float, ? super Integer, ? extends Integer> remappingFunction)	{
    	int index = index (key);
    	Integer value = null;
    	if (index > 0)	value = remappingFunction.apply((Float)key, (Integer)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public int comptueIfAbsent (float key, Function<? super Float,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public int computeIfPresent (float key,
    				BiFunction<? super Float, ? super Integer, ? extends Integer> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TFloatIntMap that;
    	int min, max;
    	Equals1 (TFloatIntMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TFloatIntMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Float, ? super Integer> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Float, ? super Integer> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Float, ? super Integer> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TFloatIntHashMapSpliterator {
        final TFloatIntHashMap map;
        ...
        TFloatIntHashMapSpliterator(TFloatIntHashMap m, int origin,
                           int fence, int est, float no_entry_key, int no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TFloatIntKeySpliterator
        extends TFloatIntHashMapSpliterator
        implements TFloatSpliterator {
        TFloatIntKeySpliterator(TFloatIntHashMap m, int origin, int fence, int est, float no_entry, int no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TFloatIntKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TFloatIntKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TFloatIntValueSpliterator
        extends TFloatIntHashMapSpliterator
        implements TIntSpliterator {
        TFloatIntValueSpliterator(TFloatIntHashMap m, int origin, int fence, int est,
                         float no_entry_key, int no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TFloatIntValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TFloatIntValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TFloatSet {
        ...
        public TFloatSpliterator spliterator() {
        	return new TFloatIntKeySpliterator(TFloatIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public float[] toArrayPar (int numProcs) {
            return TFloatIntHashMap.this.keysPar(numProcs);
        }
        ...
        public float[] toArrayPar (float[] dest, int numProcs)	{
        	return TFloatIntHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatIntHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatIntHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TFloatIntHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TFloatIntHashMap.this.index((Float)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TFloatIntHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Float> action)	{
        	TFloatIntHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TFloatIntHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public int merge (float key, int value, BiFunction<? super Float, ? super Integer,
    		? extends Integer> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public int putPar (float key, int value) {
    ...
    private int doPut (float key, int value, int index) {
      ...
      * with key
      */
     public int putParAll (float key, int value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private int doPutAll(float key, int value, int index) {
        int previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Float, ? extends Integer> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Float, ? extends Integer> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	float[] keys;
     	int[] values;
     	PutAll2 (int a, int b, float[] c, int[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TFloatIntMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (float[] keys, int[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(float[] keys, int[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Float, ? super Integer, ? extends Integer> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Float, ? super Integer,
			? extends Integer> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Float, ? super Integer,
    ...
    protected class TValueView implements TIntCollection {
        ...
        public TIntSpliterator spliterator() {
        	return new TFloatIntValueSpliterator(TFloatIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(int entry, int numProcs) {
            return TFloatIntHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public int[] toArray() {
            return TFloatIntHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public int[] toArrayPar(int numProcs) {
            return TFloatIntHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public int[] toArray(int[] dest) {
            return TFloatIntHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public int[] toArrayPar(int[] dest, int numProcs) {
            return TFloatIntHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int[] values = TFloatIntHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Integer> action)	{
        	TFloatIntHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TFloatIntHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	int[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, float[] oldK, int[] oldV, byte[] oldS){
