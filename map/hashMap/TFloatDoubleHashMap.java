...
public class TFloatDoubleHashMap extends TFloatDoubleHash implements TFloatDoubleMap, Serializable, Cloneable {
     ...
     */
    public TFloatDoubleHashMap (float[] keys, double[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TFloatDoubleHashMap(TFloatDoubleMap map, int numProcs) {
        super(map.size());
        if (map instanceof TFloatDoubleHashMap) {
            TFloatDoubleHashMap hashmap = (TFloatDoubleHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TFloatDoubleHashMap clone() {
        TFloatDoubleHashMap result;
        try {
            result = (TFloatDoubleHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TFloatDoubleHashMap clonePar(int numProcs) {
        TFloatDoubleHashMap result;
        try {
            result = (TFloatDoubleHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public double compute (float key,
    	...
    	int index = index (key);
    	Double value = null;
    	if (index > 0)	value = remappingFunction.apply((Float)key, (Double)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public double comptueIfAbsent (float key, Function<? super Float,
    ...
    private class Equals1 implements Runnable	{
    	TFloatDoubleMap that;
    	int min, max;
    	Equals1 (TFloatDoubleMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TFloatDoubleMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Float, ? super Double> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Float, ? super Double> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Float, ? super Double> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TFloatDoubleHashMapSpliterator {
        final TFloatDoubleHashMap map;
        ...
        TFloatDoubleHashMapSpliterator(TFloatDoubleHashMap m, int origin,
                           int fence, int est, float no_entry_key, double no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TFloatDoubleKeySpliterator
        extends TFloatDoubleHashMapSpliterator
        implements TFloatSpliterator {
        TFloatDoubleKeySpliterator(TFloatDoubleHashMap m, int origin, int fence, int est, float no_entry, double no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TFloatDoubleKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TFloatDoubleKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TFloatDoubleValueSpliterator
        extends TFloatDoubleHashMapSpliterator
        implements TDoubleSpliterator {
        TFloatDoubleValueSpliterator(TFloatDoubleHashMap m, int origin, int fence, int est,
                         float no_entry_key, double no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TFloatDoubleValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TFloatDoubleValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TFloatSet {
        ...
        public TFloatSpliterator spliterator() {
        	return new TFloatDoubleKeySpliterator(TFloatDoubleHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public float[] toArrayPar (int numProcs) {
            return TFloatDoubleHashMap.this.keysPar(numProcs);
        }
        ...
        public float[] toArrayPar (float[] dest, int numProcs)	{
        	return TFloatDoubleHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatDoubleHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatDoubleHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TFloatDoubleHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TFloatDoubleHashMap.this.index((Float)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TFloatDoubleHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Float> action)	{
        	TFloatDoubleHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TFloatDoubleHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public double merge (float key, double value, BiFunction<? super Float, ? super Double,
    		? extends Double> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public double putPar (float key, double value) {
    ...
    private double doPut (float key, double value, int index) {
      ...
      * with key
      */
     public double putParAll (float key, double value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private double doPutAll(float key, double value, int index) {
        double previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Float, ? extends Double> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Float, ? extends Double> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	float[] keys;
     	double[] values;
     	PutAll2 (int a, int b, float[] c, double[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TFloatDoubleMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (float[] keys, double[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(float[] keys, double[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Float, ? super Double, ? extends Double> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Float, ? super Double,
			? extends Double> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Float, ? super Double,
    ...
    protected class TValueView implements TDoubleCollection {
        ...
        public TDoubleSpliterator spliterator() {
        	return new TFloatDoubleValueSpliterator(TFloatDoubleHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(double entry, int numProcs) {
            return TFloatDoubleHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public double[] toArray() {
            return TFloatDoubleHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public double[] toArrayPar(int numProcs) {
            return TFloatDoubleHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public double[] toArray(double[] dest) {
            return TFloatDoubleHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public double[] toArrayPar(double[] dest, int numProcs) {
            return TFloatDoubleHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	double[] values = TFloatDoubleHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Double> action)	{
        	TFloatDoubleHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Double> action, int numProcs)	{
        	TFloatDoubleHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	double[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, float[] oldK, double[] oldV, byte[] oldS){
