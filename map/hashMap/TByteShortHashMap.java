...
public class TByteShortHashMap extends TByteShortHash implements TByteShortMap, Serializable, Cloneable {
     ...
     */
    public TByteShortHashMap (byte[] keys, short[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TByteShortHashMap(TByteShortMap map, int numProcs) {
        super(map.size());
        if (map instanceof TByteShortHashMap) {
            TByteShortHashMap hashmap = (TByteShortHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TByteShortHashMap clone() {
        TByteShortHashMap result;
        try {
            result = (TByteShortHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TByteShortHashMap clonePar(int numProcs) {
        TByteShortHashMap result;
        try {
            result = (TByteShortHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public short compute (byte key,
    	...
    	int index = index (key);
    	Short value = null;
    	if (index > 0)	value = remappingFunction.apply((Byte)key, (Short)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public short comptueIfAbsent (byte key, Function<? super Byte,
    ...
    private class Equals1 implements Runnable	{
    	TByteShortMap that;
    	int min, max;
    	Equals1 (TByteShortMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TByteShortMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Byte, ? super Short> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Byte, ? super Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Byte, ? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TByteShortHashMapSpliterator {
        final TByteShortHashMap map;
        ...
        TByteShortHashMapSpliterator(TByteShortHashMap m, int origin,
                           int fence, int est, byte no_entry_key, short no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TByteShortKeySpliterator
        extends TByteShortHashMapSpliterator
        implements TByteSpliterator {
        TByteShortKeySpliterator(TByteShortHashMap m, int origin, int fence, int est, byte no_entry, short no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TByteShortKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TByteShortKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TByteShortValueSpliterator
        extends TByteShortHashMapSpliterator
        implements TShortSpliterator {
        TByteShortValueSpliterator(TByteShortHashMap m, int origin, int fence, int est,
                         byte no_entry_key, short no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TByteShortValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TByteShortValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TByteSet {
        ...
        public TByteSpliterator spliterator() {
        	return new TByteShortKeySpliterator(TByteShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public byte[] toArrayPar (int numProcs) {
            return TByteShortHashMap.this.keysPar(numProcs);
        }
        ...
        public byte[] toArrayPar (byte[] dest, int numProcs)	{
        	return TByteShortHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteShortHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TByteShortHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TByteShortHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TByteShortHashMap.this.index((Byte)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TByteShortHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Byte> action)	{
        	TByteShortHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TByteShortHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public short merge (byte key, short value, BiFunction<? super Byte, ? super Short,
    		? extends Short> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public short putPar (byte key, short value) {
    ...
    private short doPut (byte key, short value, int index) {
      ...
      * with key
      */
     public short putParAll (byte key, short value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private short doPutAll(byte key, short value, int index) {
        short previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Byte, ? extends Short> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Byte, ? extends Short> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	byte[] keys;
     	short[] values;
     	PutAll2 (int a, int b, byte[] c, short[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TByteShortMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (byte[] keys, short[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(byte[] keys, short[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Byte, ? super Short, ? extends Short> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Byte, ? super Short,
			? extends Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Byte, ? super Short,
    ...
    protected class TValueView implements TShortCollection {
        ...
        public TShortSpliterator spliterator() {
        	return new TByteShortValueSpliterator(TByteShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(short entry, int numProcs) {
            return TByteShortHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public short[] toArray() {
            return TByteShortHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public short[] toArrayPar(int numProcs) {
            return TByteShortHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public short[] toArray(short[] dest) {
            return TByteShortHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public short[] toArrayPar(short[] dest, int numProcs) {
            return TByteShortHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	short[] values = TByteShortHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Short> action)	{
        	TByteShortHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TByteShortHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	short[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, byte[] oldK, short[] oldV, byte[] oldS){
