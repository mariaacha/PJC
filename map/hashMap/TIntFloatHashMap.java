...
public class TIntFloatHashMap extends TIntFloatHash implements TIntFloatMap, Serializable, Cloneable {
     ...
     */
    public TIntFloatHashMap (int[] keys, float[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TIntFloatHashMap(TIntFloatMap map, int numProcs) {
        super(map.size());
        if (map instanceof TIntFloatHashMap) {
            TIntFloatHashMap hashmap = (TIntFloatHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TIntFloatHashMap clone() {
        TIntFloatHashMap result;
        try {
            result = (TIntFloatHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TIntFloatHashMap clonePar(int numProcs) {
        TIntFloatHashMap result;
        try {
            result = (TIntFloatHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public float compute (int key,
    			BiFunction<? super Integer, ? super Float, ? extends Float> remappingFunction)	{
    	int index = index (key);
    	Float value = null;
    	if (index > 0)	value = remappingFunction.apply((Integer)key, (Float)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public float comptueIfAbsent (int key, Function<? super Integer,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public float computeIfPresent (int key,
    				BiFunction<? super Integer, ? super Float, ? extends Float> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TIntFloatMap that;
    	int min, max;
    	Equals1 (TIntFloatMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TIntFloatMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Integer, ? super Float> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Integer, ? super Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Integer, ? super Float> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TIntFloatHashMapSpliterator {
        final TIntFloatHashMap map;
        ...
        TIntFloatHashMapSpliterator(TIntFloatHashMap m, int origin,
                           int fence, int est, int no_entry_key, float no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TIntFloatKeySpliterator
        extends TIntFloatHashMapSpliterator
        implements TIntSpliterator {
        TIntFloatKeySpliterator(TIntFloatHashMap m, int origin, int fence, int est, int no_entry, float no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TIntFloatKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TIntFloatKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TIntFloatValueSpliterator
        extends TIntFloatHashMapSpliterator
        implements TFloatSpliterator {
        TIntFloatValueSpliterator(TIntFloatHashMap m, int origin, int fence, int est,
                         int no_entry_key, float no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TIntFloatValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TIntFloatValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TIntSet {
        ...
        public TIntSpliterator spliterator() {
        	return new TIntFloatKeySpliterator(TIntFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public int[] toArrayPar (int numProcs) {
            return TIntFloatHashMap.this.keysPar(numProcs);
        }
        ...
        public int[] toArrayPar (int[] dest, int numProcs)	{
        	return TIntFloatHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntFloatHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntFloatHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TIntFloatHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TIntFloatHashMap.this.index((Integer)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TIntFloatHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Integer> action)	{
        	TIntFloatHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TIntFloatHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public float merge (int key, float value, BiFunction<? super Integer, ? super Float,
    		? extends Float> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public float putPar (int key, float value) {
    ...
    private float doPut (int key, float value, int index) {
      ...
      * with key
      */
     public float putParAll (int key, float value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private float doPutAll(int key, float value, int index) {
        float previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Integer, ? extends Float> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Integer, ? extends Float> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	int[] keys;
     	float[] values;
     	PutAll2 (int a, int b, int[] c, float[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TIntFloatMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(int[] keys, float[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Integer, ? super Float, ? extends Float> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Integer, ? super Float,
			? extends Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Integer, ? super Float,
    ...
    protected class TValueView implements TFloatCollection {
        ...
        public TFloatSpliterator spliterator() {
        	return new TIntFloatValueSpliterator(TIntFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(float entry, int numProcs) {
            return TIntFloatHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public float[] toArray() {
            return TIntFloatHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public float[] toArrayPar(int numProcs) {
            return TIntFloatHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public float[] toArray(float[] dest) {
            return TIntFloatHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public float[] toArrayPar(float[] dest, int numProcs) {
            return TIntFloatHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	float[] values = TIntFloatHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Float> action)	{
        	TIntFloatHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TIntFloatHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	float[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, int[] oldK, float[] oldV, byte[] oldS){
