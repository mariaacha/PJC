...
//import paralel.collection.TFloatCollection;
//import paralel.collection.TObjectCollection;
//import paralel.iterator.TFloatIterator;
//import paralel.spliterator.TFloatSpliterator;
...
import PJC.hash.TFloatFloatHash;
import PJC.iterator.TFloatIterator;
import PJC.list.ArraysPar;
import PJC.map.TFloatFloatMap;
import PJC.map.hashMap.HashFunctions;
import PJC.map.iterator.TFloatFloatIterator;
import PJC.set.TFloatSet;
import PJC.spliterator.TFloatSpliterator;

public class TFloatFloatHashMap extends TFloatFloatHash implements TFloatFloatMap, Serializable, Cloneable {
     ...
     */
    public TFloatFloatHashMap (float[] keys, float[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TFloatFloatHashMap(TFloatFloatMap map, int numProcs) {
        super(map.size());
        if (map instanceof TFloatFloatHashMap) {
            TFloatFloatHashMap hashmap = (TFloatFloatHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TFloatFloatHashMap clone() {
        TFloatFloatHashMap result;
        try {
            result = (TFloatFloatHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TFloatFloatHashMap clonePar(int numProcs) {
        TFloatFloatHashMap result;
        try {
            result = (TFloatFloatHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public float compute (float key,
    	...
    	int index = index (key);
    	Float value = null;
    	if (index > 0)	value = remappingFunction.apply((Float)key, (Float)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public float comptueIfAbsent (float key, Function<? super Float,
    ...
    private class Equals1 implements Runnable	{
    	TFloatFloatMap that;
    	int min, max;
    	Equals1 (TFloatFloatMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TFloatFloatMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Float, ? super Float> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Float, ? super Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Float, ? super Float> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TFloatFloatHashMapSpliterator {
        final TFloatFloatHashMap map;
        ...
        TFloatFloatHashMapSpliterator(TFloatFloatHashMap m, int origin,
                           int fence, int est, float no_entry_key, float no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TFloatFloatKeySpliterator
        extends TFloatFloatHashMapSpliterator
        implements TFloatSpliterator {
        TFloatFloatKeySpliterator(TFloatFloatHashMap m, int origin, int fence, int est, float no_entry, float no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TFloatFloatKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TFloatFloatKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TFloatFloatValueSpliterator
        extends TFloatFloatHashMapSpliterator
        implements TFloatSpliterator {
        TFloatFloatValueSpliterator(TFloatFloatHashMap m, int origin, int fence, int est,
                         float no_entry_key, float no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TFloatFloatValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TFloatFloatValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TFloatSet {
        ...
        public TFloatSpliterator spliterator() {
        	return new TFloatFloatKeySpliterator(TFloatFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public float[] toArrayPar (int numProcs) {
            return TFloatFloatHashMap.this.keysPar(numProcs);
        }
        ...
        public float[] toArrayPar (float[] dest, int numProcs)	{
        	return TFloatFloatHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatFloatHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatFloatHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TFloatFloatHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TFloatFloatHashMap.this.index((Float)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TFloatFloatHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Float> action)	{
        	TFloatFloatHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TFloatFloatHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public float merge (float key, float value, BiFunction<? super Float, ? super Float,
    		? extends Float> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public float putPar (float key, float value) {
    ...
    private float doPut (float key, float value, int index) {
      ...
      * with key
      */
     public float putParAll (float key, float value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private float doPutAll(float key, float value, int index) {
        float previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Float, ? extends Float> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Float, ? extends Float> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	float[] keys;
     	float[] values;
     	PutAll2 (int a, int b, float[] c, float[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TFloatFloatMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (float[] keys, float[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(float[] keys, float[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Float, ? super Float, ? extends Float> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Float, ? super Float,
			? extends Float> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Float, ? super Float,
    ...
    protected class TValueView implements TFloatCollection {
        ...
        /** {@inheritDoc} */
        public TFloatSpliterator spliterator() {
        	return new TFloatFloatValueSpliterator(TFloatFloatHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        /** {@inheritDoc} */
        public boolean containsPar(float entry, int numProcs) {
            return TFloatFloatHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public float[] toArray() {
            return TFloatFloatHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public float[] toArrayPar(int numProcs) {
            return TFloatFloatHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public float[] toArray(float[] dest) {
            return TFloatFloatHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public float[] toArrayPar(float[] dest, int numProcs) {
            return TFloatFloatHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	float[] values = TFloatFloatHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        /** {@inheritDoc} */
        public void forEach (Consumer<? super Float> action)	{
        	TFloatFloatHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TFloatFloatHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	float[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, float[] oldK, float[] oldV, byte[] oldS){
