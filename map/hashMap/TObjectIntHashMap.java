...
public class TObjectIntHashMap extends TObjectIntHash implements TObjectIntMap, Serializable, Cloneable {
     ...
     *
     * @param initialCapacity an <code>int</code> value
     * @param loadFactor a <code>int</code> value
     ...
     */
    public TObjectIntHashMap (int initialCapacity, float loadFactor,
        Object noEntryKey, int noEntryValue) {
        super(initialCapacity, loadFactor, noEntryKey, noEntryValue);
    }


    /**
     * Creates a new <code>TObjectIntHashMap</code> instance containing
     ...
     */
    public TObjectIntHashMap (Object[] keys, int[] values) {
     ...
     */
    public TObjectIntHashMap (Object[] keys, int[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * all of the entries in the map passed in.
     *
     * @param map a <tt>TObjectIntMap</tt> that will be duplicated.
     */
    public TObjectIntHashMap (TObjectIntMap map) {
        super(map.size());
     ...
     * @param numProcs number of processors that will be used
     */
    public TObjectIntHashMap(TObjectIntMap map, int numProcs) {
        super(map.size());
        if (map instanceof TObjectIntHashMap) {
            TObjectIntHashMap hashmap = (TObjectIntHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TObjectIntHashMap clone() {
        TObjectIntHashMap result;
        try {
            result = (TObjectIntHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TObjectIntHashMap clonePar(int numProcs) {
        TObjectIntHashMap result;
        try {
            result = (TObjectIntHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public int compute (Object key,
    			BiFunction<? super Object, ? super Integer, ? extends Integer> remappingFunction)	{
    	int index = index (key);
    	Integer value = null;
    	if (index > 0)	value = remappingFunction.apply((Object)key, (Integer)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
    ...
    public int comptueIfAbsent (Object key, Function<? super Object,
    	...
    	int index = index(key);
    	Integer value = remappingFunction.apply(key);
    	if (value == null)	{
    		return no_entry_value;
    	}
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public int computeIfPresent (Object key,
    				BiFunction<? super Object, ? super Integer, ? extends Integer> remappingFunction)	{
    	...
    	if ((index> 0) && (_values[index] != no_entry_value))	{
    		int value = remappingFunction.apply(key, _values[index]);
    ...
    private class Equals1 implements Runnable	{
    	TObjectIntMap that;
    	int min, max;
    	Equals1 (TObjectIntMap obj, int a, int b)	{
    	...
    	public void run ()	{
    		...
    		for (int i=min; i<max; i++) {
                if (_states[i] == FULL) {
                    ...
                    if (!(that_value instanceof Integer))	{
                    	boolGlobal = false;
                    	return;
                    }
                    int this_value = _values[i];
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TObjectIntMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Object, ? super Integer> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Object, ? super Integer> c)	{
    ...
    static class TObjectIntHashMapSpliterator {
        final TObjectIntHashMap map;
        ...
        int _no_entry_value;			// int value that represents null

        TObjectIntHashMapSpliterator(TObjectIntHashMap m, int origin,
                           int fence, int est, Object no_entry_key, int no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TObjectIntKeySpliterator
        extends TObjectIntHashMapSpliterator
        implements TObjectSpliterator {
        TObjectIntKeySpliterator(TObjectIntHashMap m, int origin, int fence, int est, Object no_entry, int no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TObjectIntKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TObjectIntKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TObjectIntValueSpliterator
        extends TObjectIntHashMapSpliterator
        implements TIntSpliterator {
        TObjectIntValueSpliterator(TObjectIntHashMap m, int origin, int fence, int est,
                         Object no_entry_key, int no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TObjectIntValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TObjectIntValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TObjectSet {
        ...
        public TObjectIterator iterator() {
            return new TObjectIntKeyHashIterator (TObjectIntHashMap.this);
        }
        
        /** {@inheritDoc} */
        public TObjectSpliterator spliterator() {
        	return new TObjectIntKeySpliterator(TObjectIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean contains (Object entry) {
            return TObjectIntHashMap.this.contains(entry);
        }
        ...
        public Object[] toArray() {
            return TObjectIntHashMap.this.keys();
        }

        /** {@inheritDoc} */
        public Object[] toArrayPar (int numProcs) {
            return TObjectIntHashMap.this.keysPar(numProcs);
        }

        /** {@inheritDoc} */
        public Object[] toArray (Object[] dest) {
            return TObjectIntHashMap.this.keys(dest);
        }
        public Object[] toArrayPar (Object[] dest, int numProcs)	{
        	return TObjectIntHashMap.this.keysPar(dest, numProcs);
        }


        /**
         * Unsupported when operating upon a Key Set view of a TObjectIntMap
         * <p/>
         * {@inheritDoc}
         */
        ...
        public boolean remove (Object entry) {
            return no_entry_value != TObjectIntHashMap.this.remove(entry);
        }
        /** {@inheritDoc} */
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
                    ...
                    if (array[i] instanceof Object) {
                    	...
                    	Object ele = array[i];
                        if (!TObjectIntHashMap.this.containsKey(ele)) {
                            boolGlobal = false;
                            return;
                        }
                    } 
        ...
        public boolean containsAll (Collection<?> collection) {
            for (Object element : collection) {
                if (element instanceof Object) {
                    ...
                    if (!TObjectIntHashMap.this.containsKey(element)) {
                        return false;
                    }
                } else {
                    return false;
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectIntHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (TObjectCollection collection) {
            ...
            while (iter.hasNext()) {
                if (!TObjectIntHashMap.this.containsKey(iter.next())) {
                    return false;
                }
            }
            return true;
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectIntHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (Object[] array) {
            for (Object element : array) {
                if (!TObjectIntHashMap.this.contains(element)) {
                    return false;
                }
            }
            return true;
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TObjectIntHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TObjectIntHashMap.this.index((Object)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TObjectIntHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Object> action)	{
        	TObjectIntHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Object> action, int numProcs)	{
        	TObjectIntHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    public int merge (Object key, int value, BiFunction<? super Object, ? super Integer,
    	...
    	Integer newValue = remappingFunction.apply (_set[index], value);
    	if (newValue != null)	{
    		put (key, newValue);
    		return newValue;
    	}
     ...
     * with key
     */
     public int putPar (Object key, int value) {
     ...
     */
    private int doPut (Object key, int value, int index) {
      ...
      * with key
      */
     public int putParAll (Object key, int value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private int doPutAll(Object key, int value, int index) {
        int previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Object, ? extends Integer> map) {
        ...
        for (Map.Entry<? extends Object, ? extends Integer> entry : map.entrySet()) {
            //this.put(entry.getKey().ObjectValue(), entry.getValue().intValue());
        	this.put(entry.getKey(), entry.getValue().intValue());
        }
    }
     	...
     	public void run ()	{
     		...
     		for (int i=min; i<max; i++)	{
     			if (entradas[i].getKey() instanceof Integer)	{
     				...
     				valor = ((Integer) entradas[i].getValue()).intValue();
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Object, ? extends Integer> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	Object[] keys;
     	int[] values;
     	PutAll2 (int a, int b, Object[] c, int[] d)	{
     ...
     /** {@inheritDoc} */
     public void putAll (TObjectIntMap map) {
         ensureCapacity(map.size());
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TObjectIntMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
         Object[] set = map.keySet().toArrayPar(numProcs);
         int[] values = map.values().toArrayPar(numProcs);
         int size = set.length;
    ...
    public void replaceAll (BiFunction 
    			<? super Object, ? super Integer, ? extends Integer> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	int min, max;
    	BiFunction <? super Object, ? super Integer, ? extends Integer> function;
    	ReplaceAll1 (int a, int b, BiFunction <? super Object, ? super Integer,
			? extends Integer> c)	{
    ...
    public TIntCollection values ()	{
    	TIntCollection c = new TValueView ();
    	return c;
    }
    ...
    protected class TValueView implements TIntCollection {
        ...
        public TIntIterator iterator() {
            return new TObjectIntValueHashIterator(TObjectIntHashMap.this);
        }

        /** {@inheritDoc} */
        public TIntSpliterator spliterator() {
        	return new TObjectIntValueSpliterator(TObjectIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean equalsPar2 (Object o, int numProcs)	{
        	boolGlobal = false;
        	if (o instanceof TIntCollection)	{
        		TIntCollection that = (TIntCollection) o;
        		if (this.size() != that.size())	{
        			return false;
        		}
        		threads = new Thread[numProcs];
        		...
        		int numElemProc = _values.length/numProcs;
        		int numMin, numMax;
        		int[] array = that.toArrayPar(numProcs);
        		for (int i=0; i<numProcs; i++)	{
        			numMin = limiteMin (i, numElemProc, 0);
            		numMax = limiteMax (i, numElemProc, 0, numProcs, _values.length);
        ...
        private class Equals3 implements Runnable	{
        	int min, max;
        	TIntCollection that;
        	Equals3 (int a, int b, TIntCollection c)	{
        		min = a; max = b; that = c;
        	}
        	public void run ()	{
        		for (int i=min; i<max; i++)	{
        ...
        public boolean containsPar(int entry, int numProcs) {
            return TObjectIntHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public int[] toArray() {
            return TObjectIntHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public int[] toArrayPar(int numProcs) {
            return TObjectIntHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public int[] toArray(int[] dest) {
            return TObjectIntHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public int[] toArrayPar(int[] dest, int numProcs) {
            return TObjectIntHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        private class ContainsAll1 implements Runnable	{
        	...
        	Object[] array;
        	int[] values;
        	ContainsAll1 (int a, int b, Object[] c, int[] d)	{
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int numElemProc = collection.size()/numProcs;
        	Object[] array = collection.toArray (new Object[collection.size()]);
        	int[] values = TObjectIntHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        private class RetainAll1 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				if (Arrays.binarySearch(array, (Integer)_values[i]) < 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        			}
        ...
        public void forEach (Consumer<? super Integer> action)	{
        	TObjectIntHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TObjectIntHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    class TObjectIntKeyHashIterator extends THashPrimitiveIterator implements TObjectIterator {
         ...
         */
        TObjectIntKeyHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	int[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, Object[] oldK, int[] oldV, byte[] oldS){
    ...
    private class ToArrayValues1 implements Runnable	{
    	...
    	public void run()	{
    		...
    		if (idProc > 0)	{
    			...
    			System.arraycopy(intsLocal, 0, intsGlobal, k, j);
    		}
    		else	{
    ...
    /** {@inheritDoc} */
    protected int[] toArrayValuesPar (int numProcs)	{
    	atIntGlobal = new AtomicInteger(0);
    ...
    /** {@inheritDoc} */
    protected int[] toArrayValuesPar (int[] array, int numProcs)	{
    	if (array.length < size())
    		array = new int[size()];
    	intsGlobal = new int[_size];
    	intsGlobal2 = new int[numProcs];
    	atIntGlobal = new AtomicInteger(0);
    	int numElemProc = _values.length/numProcs;
    	int numMin, numMax;
    ...
    /** {@inheritDoc} */
    protected int[] toArrayValues (int[] array) {
        int size = size();
        if (array.length < size) {
            array = new int[size];
        }
    ...
    private class ForEachValue1 implements Runnable	{
    	...
    	ForEachValue1 (int a, int b, Consumer<? super Integer> c)	{
    		min = a; max = b; action = c;
    	}
    	public void run ()	{
    		for (int i=min; i<max; i++)	{
    ...
    /** {@inheritDoc} */
    protected void forEachValuePar (Consumer<? super Integer> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0)	{
    		int numElemProc = _values.length/numProcs;
    ...
    /** {@inheritDoc} */
    protected void forEachValue (Consumer<? super Integer> action)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0)	{
    		for (int i=0; i<_values.length; i++)	{
    ...
    /** {@inheritDoc} */
    public int adjustOrPutValue (Object key, int adjust_amount, int put_amount) {
        int index = insertKey(key);
    ...
    class TObjectIntValueHashIterator extends THashPrimitiveIterator implements TIntIterator {
         ...
         * @param hash the <tt>TPrimitiveHash</tt> we will be iterating over.
         */
        TObjectIntValueHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    class TObjectIntHashIterator extends THashPrimitiveIterator implements TObjectIntIterator {

        /**
         * Creates an iterator over the specified map
         *
         * @param map the <tt>TObjectIntHashMap</tt> we will be iterating over.
         */
        TObjectIntHashIterator (TObjectIntHashMap map) {
            super(map);
        }
