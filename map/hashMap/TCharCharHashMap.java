...
//import paralel.collection.TCharCollection;
//import paralel.collection.TObjectCollection;
//import paralel.iterator.TCharIterator;
//import paralel.spliterator.TCharSpliterator;
...
import PJC.hash.TCharCharHash;
import PJC.iterator.TCharIterator;
import PJC.list.ArraysPar;
import PJC.map.TCharCharMap;
import PJC.map.hashMap.HashFunctions;
import PJC.map.iterator.TCharCharIterator;
import PJC.set.TCharSet;
import PJC.spliterator.TCharSpliterator;

public class TCharCharHashMap extends TCharCharHash implements TCharCharMap, Serializable, Cloneable {
     ...
     */
    public TCharCharHashMap (char[] keys, char[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TCharCharHashMap(TCharCharMap map, int numProcs) {
        super(map.size());
        if (map instanceof TCharCharHashMap) {
            TCharCharHashMap hashmap = (TCharCharHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TCharCharHashMap clone() {
        TCharCharHashMap result;
        try {
            result = (TCharCharHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TCharCharHashMap clonePar(int numProcs) {
        TCharCharHashMap result;
        try {
            result = (TCharCharHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public char compute (char key,
    			BiFunction<? super Character, ? super Character, ? extends Character> remappingFunction)	{
    	int index = index (key);
    	Character value = null;
    	if (index > 0)	value = remappingFunction.apply((Character)key, (Character)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public char comptueIfAbsent (char key, Function<? super Character,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public char computeIfPresent (char key,
    				BiFunction<? super Character, ? super Character, ? extends Character> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TCharCharMap that;
    	int min, max;
    	Equals1 (TCharCharMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TCharCharMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Character, ? super Character> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Character, ? super Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Character, ? super Character> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TCharCharHashMapSpliterator {
        final TCharCharHashMap map;
        ...
        TCharCharHashMapSpliterator(TCharCharHashMap m, int origin,
                           int fence, int est, char no_entry_key, char no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TCharCharKeySpliterator
        extends TCharCharHashMapSpliterator
        implements TCharSpliterator {
        TCharCharKeySpliterator(TCharCharHashMap m, int origin, int fence, int est, char no_entry, char no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TCharCharKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TCharCharKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TCharCharValueSpliterator
        extends TCharCharHashMapSpliterator
        implements TCharSpliterator {
        TCharCharValueSpliterator(TCharCharHashMap m, int origin, int fence, int est,
                         char no_entry_key, char no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TCharCharValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TCharCharValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TCharSet {
        ...
        public TCharSpliterator spliterator() {
        	return new TCharCharKeySpliterator(TCharCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public char[] toArrayPar (int numProcs) {
            return TCharCharHashMap.this.keysPar(numProcs);
        }
        ...
        public char[] toArrayPar (char[] dest, int numProcs)	{
        	return TCharCharHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharCharHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TCharCharHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TCharCharHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TCharCharHashMap.this.index((Character)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TCharCharHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Character> action)	{
        	TCharCharHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TCharCharHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public char merge (char key, char value, BiFunction<? super Character, ? super Character,
    		? extends Character> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public char putPar (char key, char value) {
    ...
    private char doPut (char key, char value, int index) {
      ...
      * with key
      */
     public char putParAll (char key, char value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private char doPutAll(char key, char value, int index) {
        char previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Character, ? extends Character> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Character, ? extends Character> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	char[] keys;
     	char[] values;
     	PutAll2 (int a, int b, char[] c, char[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TCharCharMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(char[] keys, char[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Character, ? super Character, ? extends Character> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Character, ? super Character,
			? extends Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Character, ? super Character,
    ...
    protected class TValueView implements TCharCollection {
        ...
        /** {@inheritDoc} */
        public TCharSpliterator spliterator() {
        	return new TCharCharValueSpliterator(TCharCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        /** {@inheritDoc} */
        public boolean containsPar(char entry, int numProcs) {
            return TCharCharHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public char[] toArray() {
            return TCharCharHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public char[] toArrayPar(int numProcs) {
            return TCharCharHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public char[] toArray(char[] dest) {
            return TCharCharHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public char[] toArrayPar(char[] dest, int numProcs) {
            return TCharCharHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	char[] values = TCharCharHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        /** {@inheritDoc} */
        public void forEach (Consumer<? super Character> action)	{
        	TCharCharHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TCharCharHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	char[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, char[] oldK, char[] oldV, byte[] oldS){
