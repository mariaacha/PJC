...
public class TFloatShortHashMap extends TFloatShortHash implements TFloatShortMap, Serializable, Cloneable {
     ...
     */
    public TFloatShortHashMap (float[] keys, short[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TFloatShortHashMap(TFloatShortMap map, int numProcs) {
        super(map.size());
        if (map instanceof TFloatShortHashMap) {
            TFloatShortHashMap hashmap = (TFloatShortHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TFloatShortHashMap clone() {
        TFloatShortHashMap result;
        try {
            result = (TFloatShortHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TFloatShortHashMap clonePar(int numProcs) {
        TFloatShortHashMap result;
        try {
            result = (TFloatShortHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public short compute (float key,
    	...
    	int index = index (key);
    	Short value = null;
    	if (index > 0)	value = remappingFunction.apply((Float)key, (Short)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public short comptueIfAbsent (float key, Function<? super Float,
    ...
    private class Equals1 implements Runnable	{
    	TFloatShortMap that;
    	int min, max;
    	Equals1 (TFloatShortMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TFloatShortMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Float, ? super Short> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Float, ? super Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Float, ? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TFloatShortHashMapSpliterator {
        final TFloatShortHashMap map;
        ...
        TFloatShortHashMapSpliterator(TFloatShortHashMap m, int origin,
                           int fence, int est, float no_entry_key, short no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TFloatShortKeySpliterator
        extends TFloatShortHashMapSpliterator
        implements TFloatSpliterator {
        TFloatShortKeySpliterator(TFloatShortHashMap m, int origin, int fence, int est, float no_entry, short no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TFloatShortKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TFloatShortKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TFloatShortValueSpliterator
        extends TFloatShortHashMapSpliterator
        implements TShortSpliterator {
        TFloatShortValueSpliterator(TFloatShortHashMap m, int origin, int fence, int est,
                         float no_entry_key, short no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TFloatShortValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TFloatShortValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TFloatSet {
        ...
        public TFloatSpliterator spliterator() {
        	return new TFloatShortKeySpliterator(TFloatShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public float[] toArrayPar (int numProcs) {
            return TFloatShortHashMap.this.keysPar(numProcs);
        }
        ...
        public float[] toArrayPar (float[] dest, int numProcs)	{
        	return TFloatShortHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatShortHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TFloatShortHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TFloatShortHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TFloatShortHashMap.this.index((Float)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TFloatShortHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Float> action)	{
        	TFloatShortHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Float> action, int numProcs)	{
        	TFloatShortHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public short merge (float key, short value, BiFunction<? super Float, ? super Short,
    		? extends Short> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public short putPar (float key, short value) {
    ...
    private short doPut (float key, short value, int index) {
      ...
      * with key
      */
     public short putParAll (float key, short value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private short doPutAll(float key, short value, int index) {
        short previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Float, ? extends Short> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Float, ? extends Short> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	float[] keys;
     	short[] values;
     	PutAll2 (int a, int b, float[] c, short[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TFloatShortMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (float[] keys, short[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(float[] keys, short[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Float, ? super Short, ? extends Short> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Float, ? super Short,
			? extends Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Float, ? super Short,
    ...
    protected class TValueView implements TShortCollection {
        ...
        public TShortSpliterator spliterator() {
        	return new TFloatShortValueSpliterator(TFloatShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(short entry, int numProcs) {
            return TFloatShortHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public short[] toArray() {
            return TFloatShortHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public short[] toArrayPar(int numProcs) {
            return TFloatShortHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public short[] toArray(short[] dest) {
            return TFloatShortHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public short[] toArrayPar(short[] dest, int numProcs) {
            return TFloatShortHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	short[] values = TFloatShortHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Short> action)	{
        	TFloatShortHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TFloatShortHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	short[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, float[] oldK, short[] oldV, byte[] oldS){
