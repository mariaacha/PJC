...
//import paralel.collection.TObjectCollection;
//import paralel.iterator.TObjectIterator;
//import paralel.spliterator.TObjectSpliterator;
...
import PJC.hash.TObjectObjectHash;
import PJC.iterator.TObjectIterator;
import PJC.list.ArraysPar;
import PJC.map.TObjectObjectMap;
import PJC.map.hashMap.HashFunctions;
import PJC.map.iterator.TObjectObjectIterator;
import PJC.set.TObjectSet;
import PJC.spliterator.TObjectSpliterator;

public class TObjectObjectHashMap extends TObjectObjectHash implements TObjectObjectMap, Serializable, Cloneable {
    ...
    /**
     * Creates a new <code>TObjectObjectHashMap</code> instance with the default
     * capacity and load factor.
     */
    public TObjectObjectHashMap() {
        super();
    }


    /**
     * Creates a new <code>TObjectObjectHashMap</code> instance with a prime
     * capacity equal to or greater than <tt>initialCapacity</tt> and
     * with the default load factor.
     *
     * @param initialCapacity an <code>int</code> value
     */
    public TObjectObjectHashMap (int initialCapacity) {
     ...
     */
    public TObjectObjectHashMap (int initialCapacity, float loadFactor) {
     ...
     */
    public TObjectObjectHashMap (int initialCapacity, float loadFactor,
        Object noEntryKey, Object noEntryValue) {
        super(initialCapacity, loadFactor, noEntryKey, noEntryValue);
    }


    /**
     * Creates a new <code>TObjectObjectHashMap</code> instance containing
     ...
     */
    public TObjectObjectHashMap (Object[] keys, Object[] values) {
     ...
     */
    public TObjectObjectHashMap (Object[] keys, Object[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * all of the entries in the map passed in.
     *
     * @param map a <tt>TObjectObjectMap</tt> that will be duplicated.
     */
    public TObjectObjectHashMap (TObjectObjectMap map) {
        super(map.size());
        if (map instanceof TObjectObjectHashMap) {
            TObjectObjectHashMap hashmap = (TObjectObjectHashMap) map;
     ...
     * @param numProcs number of processors that will be used
     */
    public TObjectObjectHashMap(TObjectObjectMap map, int numProcs) {
        super(map.size());
        if (map instanceof TObjectObjectHashMap) {
            TObjectObjectHashMap hashmap = (TObjectObjectHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TObjectObjectHashMap clone() {
        TObjectObjectHashMap result;
        try {
            result = (TObjectObjectHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TObjectObjectHashMap clonePar(int numProcs) {
        TObjectObjectHashMap result;
        try {
            result = (TObjectObjectHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
    ...
    public Object compute (Object key,
    	...
    	int index = index (key);
    	Object value = null;
    	if (index > 0)	value = remappingFunction.apply((Object)key, (Object)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public Object comptueIfAbsent (Object key, Function<? super Object,
    ...
    private class Equals1 implements Runnable	{
    	TObjectObjectMap that;
    	int min, max;
    	Equals1 (TObjectObjectMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TObjectObjectMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    	if (other instanceof TObjectObjectMap)	{
    		TObjectObjectMap that = (TObjectObjectMap) other;
    ...
    private class ForEachPar1 implements Runnable	{
    	...
    	ForEachPar1 (int a, int b, BiConsumer<? super Object, ? super Object> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Object, ? super Object> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TObjectObjectHashMapSpliterator {
        final TObjectObjectHashMap map;
        ...
        TObjectObjectHashMapSpliterator(TObjectObjectHashMap m, int origin,
                           int fence, int est, Object no_entry_key, Object no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TObjectObjectKeySpliterator
        extends TObjectObjectHashMapSpliterator
        implements TObjectSpliterator {
        TObjectObjectKeySpliterator(TObjectObjectHashMap m, int origin, int fence, int est, Object no_entry, Object no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TObjectObjectKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TObjectObjectKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TObjectObjectValueSpliterator
        extends TObjectObjectHashMapSpliterator
        implements TObjectSpliterator {
        TObjectObjectValueSpliterator(TObjectObjectHashMap m, int origin, int fence, int est,
                         Object no_entry_key, Object no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TObjectObjectValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TObjectObjectValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TObjectSet {
        ...
        public TObjectIterator iterator() {
            return new TObjectObjectKeyHashIterator (TObjectObjectHashMap.this);
        }
        
        /** {@inheritDoc} */
        public TObjectSpliterator spliterator() {
        	return new TObjectObjectKeySpliterator(TObjectObjectHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean contains (Object entry) {
            return TObjectObjectHashMap.this.contains(entry);
        }
        ...
        public Object[] toArray() {
            return TObjectObjectHashMap.this.keys();
        }

        /** {@inheritDoc} */
        public Object[] toArrayPar (int numProcs) {
            return TObjectObjectHashMap.this.keysPar(numProcs);
        }

        /** {@inheritDoc} */
        public Object[] toArray (Object[] dest) {
            return TObjectObjectHashMap.this.keys(dest);
        }
        public Object[] toArrayPar (Object[] dest, int numProcs)	{
        	return TObjectObjectHashMap.this.keysPar(dest, numProcs);
        }


        /**
         * Unsupported when operating upon a Key Set view of a TObjectObjectMap
         * <p/>
         * {@inheritDoc}
         */
        ...
        public boolean remove (Object entry) {
            return no_entry_value != TObjectObjectHashMap.this.remove(entry);
        }
        /** {@inheritDoc} */
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
                    ...
                    if (array[i] instanceof Object) {
                    	...
                    	Object ele = array[i];
                        if (!TObjectObjectHashMap.this.containsKey(ele)) {
                            boolGlobal = false;
                            return;
                        }
                    } 
        ...
        public boolean containsAll (Collection<?> collection) {
            for (Object element : collection) {
                if (element instanceof Object) {
                    ...
                    if (!TObjectObjectHashMap.this.containsKey(element)) {
                        return false;
                    }
                } else {
                    return false;
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectObjectHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (TObjectCollection collection) {
            ...
            while (iter.hasNext()) {
                if (!TObjectObjectHashMap.this.containsKey(iter.next())) {
                    return false;
                }
            }
            return true;
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectObjectHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (Object[] array) {
            for (Object element : array) {
                if (!TObjectObjectHashMap.this.contains(element)) {
                    return false;
                }
            }
            return true;
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TObjectObjectHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TObjectObjectHashMap.this.index((Object)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clear() {
            TObjectObjectHashMap.this.clear();
        }
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TObjectObjectHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Object> action)	{
        	TObjectObjectHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Object> action, int numProcs)	{
        	TObjectObjectHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public Object merge (Object key, Object value, BiFunction<? super Object, ? super Object,
    		? extends Object> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public Object putPar (Object key, Object value) {
     ...
     */
    private Object doPut (Object key, Object value, int index) {
      ...
      * with key
      */
     public Object putParAll (Object key, Object value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private Object doPutAll(Object key, Object value, int index) {
        Object previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Object, ? extends Object> map) {
        ...
        for (Map.Entry<? extends Object, ? extends Object> entry : map.entrySet()) {
            //this.put(entry.getKey().ObjectValue(), entry.getValue().ObjectValue());
        	//this.put(entry.getKey(), entry.getValue().ObjectValue());
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Object, ? extends Object> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	Object[] keys;
     	Object[] values;
     	PutAll2 (int a, int b, Object[] c, Object[] d)	{
     ...
     /** {@inheritDoc} */
     public void putAll (TObjectObjectMap map) {
         ensureCapacity(map.size());
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TObjectObjectMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Object[] keys, Object[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Object, ? super Object, ? extends Object> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Object, ? super Object,
			? extends Object> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Object, ? super Object,
    ...
    protected class TValueView implements TObjectCollection {
        ...
        /** {@inheritDoc} */
        public TObjectIterator iterator() {
            return new TObjectObjectValueHashIterator(TObjectObjectHashMap.this);
        }

        /** {@inheritDoc} */
        public TObjectSpliterator spliterator() {
        	return new TObjectObjectValueSpliterator(TObjectObjectHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        /** {@inheritDoc} */
        public boolean contains (Object entry) {
            return TObjectObjectHashMap.this.containsValue(entry);
        }
        @Override
        /** {@inheritDoc} */
        public boolean containsPar(Object entry, int numProcs) {
            return TObjectObjectHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public Object[] toArray() {
            return TObjectObjectHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public Object[] toArrayPar(int numProcs) {
            return TObjectObjectHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public Object[] toArray(Object[] dest) {
            return TObjectObjectHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public Object[] toArrayPar(Object[] dest, int numProcs) {
            return TObjectObjectHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int numElemProc = collection.size()/numProcs;
        	Object[] array = collection.toArray (new Object[collection.size()]);
        	Object[] values = TObjectObjectHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public boolean containsAll(Collection<?> collection) {
            for (Object element : collection) {
                if (element instanceof Object) {
                    //Object ele = ((Object) element).ObjectValue();
                    if (!TObjectObjectHashMap.this.containsValue(element)) {
                        return false;
                    }
                } else {
                    return false;
        ...
        public boolean containsAll (TObjectCollection collection) {
            TObjectIterator iter = collection.iterator();
            while (iter.hasNext()) {
                if (!TObjectObjectHashMap.this.containsValue(iter.next())) {
                    return false;
                }
            }
            return true;
        ...
        /** {@inheritDoc} */
        public void forEach (Consumer<? super Object> action)	{
        	TObjectObjectHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Object> action, int numProcs)	{
        	TObjectObjectHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    class TObjectObjectKeyHashIterator extends THashPrimitiveIterator implements TObjectIterator {
         ...
         */
        TObjectObjectKeyHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
        ...
        public void remove() {
            ...
            try {
                ...
                TObjectObjectHashMap.this.removeAt(_index);
            }
            finally {
    ...
    private class Rehash1 implements Runnable	{
    	...
    	Object[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, Object[] oldK, Object[] oldV, byte[] oldS){
    ...
    public TObjectObjectIterator iterator() {
        return new TObjectObjectHashIterator(this);
    }
    ...
    public boolean adjustValue (Object key, Object amount) {
        int index = index(key);
        if (index < 0) {
            return false;
        } 
    ...
    public Object adjustOrPutValue (Object key, Object adjust_amount, Object put_amount) {
        int index = insertKey(key);
    ...
    class TObjectObjectValueHashIterator extends THashPrimitiveIterator implements TObjectIterator {
         ...
         * @param hash the <tt>TPrimitiveHash</tt> we will be iterating over.
         */
        TObjectObjectValueHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    class TObjectObjectHashIterator extends THashPrimitiveIterator implements TObjectObjectIterator {

        /**
         * Creates an iterator over the specified map
         *
         * @param map the <tt>TObjectObjectHashMap</tt> we will be iterating over.
         */
        TObjectObjectHashIterator (TObjectObjectHashMap map) {
            super(map);
        }
