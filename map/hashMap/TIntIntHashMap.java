...
import PJC.hash.TIntIntHash;
import PJC.iterator.TIntIterator;
import PJC.list.ArraysPar;
import PJC.map.TIntIntMap;
import PJC.map.hashMap.HashFunctions;
import PJC.map.iterator.TIntIntIterator;
import PJC.set.TIntSet;
import PJC.spliterator.TIntSpliterator;

public class TIntIntHashMap extends TIntIntHash implements TIntIntMap, Serializable, Cloneable {
     ...
     */
    public TIntIntHashMap( int[] keys, int[] values, int numProcs) {
        super( Math.max( keys.length, values.length ) );
     ...
     * @param numProcs number of processors that will be used
     */
    public TIntIntHashMap( TIntIntMap map, int numProcs) {
        super( map.size() );
        if ( map instanceof TIntIntHashMap ) {
            TIntIntHashMap hashmap = ( TIntIntHashMap ) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TIntIntHashMap clone() {
        TIntIntHashMap result;
        try {
            result = (TIntIntHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TIntIntHashMap clonePar(int numProcs) {
        TIntIntHashMap result;
        try {
            result = (TIntIntHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     * @param remappingFunction the functio to compute a value
    ...
    private class Equals1 implements Runnable	{
    	TIntIntMap that;
    	int min, max;
    	Equals1 (TIntIntMap obj, int a, int b)	{
    		...
    		for ( int i=min; i<max; i++) {
                if ( _states[i] == FULL ) {
                    ...
                    if (!(that_value instanceof Integer))	{
                    	System.out.println ("No es entero: " + that_value);
                    	boolGlobal = false;
                    	return;
                    }
                    int this_value = _values[i];
                    if ( ( this_value != (Integer)that_value ) &&
                         ( this_value != this_no_entry_value ) &&
                         ( that_value != null ) ) {
                    	System.out.println ("Son distintos: " + this_value + ", " + that_value+". La clave es " + key);
                        boolGlobal = false;
                        return;
                    }
                }
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TIntIntMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Integer, ? super Integer> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Integer, ? super Integer> c)	{
    ...
    static class TIntIntHashMapSpliterator {
        ...
        TIntIntHashMapSpliterator(TIntIntHashMap m, int origin,
                           int fence, int est, int no_entry_key, int no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TIntIntKeySpliterator
        extends TIntIntHashMapSpliterator
        implements TIntSpliterator {
        TIntIntKeySpliterator(TIntIntHashMap m, int origin, int fence, int est, int no_entry, int no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TIntIntKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TIntIntKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TIntIntValueSpliterator
        extends TIntIntHashMapSpliterator
        implements TIntSpliterator {
        TIntIntValueSpliterator(TIntIntHashMap m, int origin, int fence, int est,
                         int no_entry_key, int no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TIntIntValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TIntIntValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TIntSet {
        ...
        public TIntSpliterator spliterator() {
        	return new TIntIntKeySpliterator(TIntIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public int[] toArrayPar (int numProcs) {
            return TIntIntHashMap.this.keysPar(numProcs);
        }
        ...
        public int[] toArrayPar (int[] dest, int numProcs)	{
        	return TIntIntHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TIntIntHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        public void forEach (Consumer<? super Integer> action)	{
        	TIntIntHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TIntIntHashMap.this.forEachKeyPar (action, numProcs);
        }
     ...
     * with key
     */
     public int putPar (int key, int value) {
    ...
    private int doPut (int key, int value, int index) {
      ...
      * with key
      */
     public int putParAll( int key, int value ) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private int doPutAll( int key, int value, int index ) {
        int previous = no_entry_value;
        //boolean isNewMapping = true;
        if ( index < 0 ) {
    ...
    public void putAll (Map<? extends Integer, ? extends Integer> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar( Map<? extends Integer, ? extends Integer> map, int numProcs ) {
         ensureCapacity( map.size() );
         atIntGlobal = new AtomicInteger(0);
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TIntIntMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
    ...
    protected class TValueView implements TIntCollection {
        ...
        /** {@inheritDoc} */
        public TIntSpliterator spliterator() {
        	return new TIntIntValueSpliterator(TIntIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        /** {@inheritDoc} */
        public boolean containsPar(int entry, int numProcs) {
            return TIntIntHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public int[] toArray() {
            return TIntIntHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public int[] toArrayPar(int numProcs) {
            return TIntIntHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public int[] toArray(int[] dest) {
            return TIntIntHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public int[] toArrayPar(int[] dest, int numProcs) {
            return TIntIntHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int[] values = TIntIntHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        /** {@inheritDoc} */
        public void forEach (Consumer<? super Integer> action)	{
        	TIntIntHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TIntIntHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    protected int[] keysPar (int[] array, int numProcs) {
    	...
    	//array = intsGlobal;
    	System.arraycopy(intsGlobal, 0, array, 0, intGlobal);
    	return intsGlobal;
    }
