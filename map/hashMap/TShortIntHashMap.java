...
public class TShortIntHashMap extends TShortIntHash implements TShortIntMap, Serializable, Cloneable {
     ...
     */
    public TShortIntHashMap (short[] keys, int[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TShortIntHashMap(TShortIntMap map, int numProcs) {
        super(map.size());
        if (map instanceof TShortIntHashMap) {
            TShortIntHashMap hashmap = (TShortIntHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TShortIntHashMap clone() {
        TShortIntHashMap result;
        try {
            result = (TShortIntHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TShortIntHashMap clonePar(int numProcs) {
        TShortIntHashMap result;
        try {
            result = (TShortIntHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public int compute (short key,
    			BiFunction<? super Short, ? super Integer, ? extends Integer> remappingFunction)	{
    	int index = index (key);
    	Integer value = null;
    	if (index > 0)	value = remappingFunction.apply((Short)key, (Integer)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public int comptueIfAbsent (short key, Function<? super Short,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public int computeIfPresent (short key,
    				BiFunction<? super Short, ? super Integer, ? extends Integer> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TShortIntMap that;
    	int min, max;
    	Equals1 (TShortIntMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TShortIntMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Short, ? super Integer> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Short, ? super Integer> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Short, ? super Integer> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TShortIntHashMapSpliterator {
        final TShortIntHashMap map;
        ...
        TShortIntHashMapSpliterator(TShortIntHashMap m, int origin,
                           int fence, int est, short no_entry_key, int no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TShortIntKeySpliterator
        extends TShortIntHashMapSpliterator
        implements TShortSpliterator {
        TShortIntKeySpliterator(TShortIntHashMap m, int origin, int fence, int est, short no_entry, int no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TShortIntKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TShortIntKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TShortIntValueSpliterator
        extends TShortIntHashMapSpliterator
        implements TIntSpliterator {
        TShortIntValueSpliterator(TShortIntHashMap m, int origin, int fence, int est,
                         short no_entry_key, int no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TShortIntValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TShortIntValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TShortSet {
        ...
        public TShortSpliterator spliterator() {
        	return new TShortIntKeySpliterator(TShortIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public short[] toArrayPar (int numProcs) {
            return TShortIntHashMap.this.keysPar(numProcs);
        }
        ...
        public short[] toArrayPar (short[] dest, int numProcs)	{
        	return TShortIntHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortIntHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TShortIntHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TShortIntHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TShortIntHashMap.this.index((Short)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TShortIntHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Short> action)	{
        	TShortIntHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TShortIntHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public int merge (short key, int value, BiFunction<? super Short, ? super Integer,
    		? extends Integer> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public int putPar (short key, int value) {
    ...
    private int doPut (short key, int value, int index) {
      ...
      * with key
      */
     public int putParAll (short key, int value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private int doPutAll(short key, int value, int index) {
        int previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Short, ? extends Integer> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Short, ? extends Integer> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	short[] keys;
     	int[] values;
     	PutAll2 (int a, int b, short[] c, int[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TShortIntMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (short[] keys, int[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(short[] keys, int[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Short, ? super Integer, ? extends Integer> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Short, ? super Integer,
			? extends Integer> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Short, ? super Integer,
    ...
    protected class TValueView implements TIntCollection {
        ...
        public TIntSpliterator spliterator() {
        	return new TShortIntValueSpliterator(TShortIntHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(int entry, int numProcs) {
            return TShortIntHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public int[] toArray() {
            return TShortIntHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public int[] toArrayPar(int numProcs) {
            return TShortIntHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public int[] toArray(int[] dest) {
            return TShortIntHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public int[] toArrayPar(int[] dest, int numProcs) {
            return TShortIntHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int[] values = TShortIntHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Integer> action)	{
        	TShortIntHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TShortIntHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	int[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, short[] oldK, int[] oldV, byte[] oldS){
