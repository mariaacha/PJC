...
public class TObjectByteHashMap extends TObjectByteHash implements TObjectByteMap, Serializable, Cloneable {
     ...
     */
    public TObjectByteHashMap (int initialCapacity, float loadFactor,
        Object noEntryKey, byte noEntryValue) {
        super(initialCapacity, loadFactor, noEntryKey, noEntryValue);
    }


    /**
     * Creates a new <code>TObjectByteHashMap</code> instance containing
     ...
     */
    public TObjectByteHashMap (Object[] keys, byte[] values) {
     ...
     */
    public TObjectByteHashMap (Object[] keys, byte[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * all of the entries in the map passed in.
     *
     * @param map a <tt>TObjectByteMap</tt> that will be duplicated.
     */
    public TObjectByteHashMap (TObjectByteMap map) {
        super(map.size());
     ...
     * @param numProcs number of processors that will be used
     */
    public TObjectByteHashMap(TObjectByteMap map, int numProcs) {
        super(map.size());
        if (map instanceof TObjectByteHashMap) {
            TObjectByteHashMap hashmap = (TObjectByteHashMap) map;
            this._loadFactor = hashmap._loadFactor;
    ...
    public TObjectByteHashMap clone() {
        TObjectByteHashMap result;
        try {
            result = (TObjectByteHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TObjectByteHashMap clonePar(int numProcs) {
        TObjectByteHashMap result;
        try {
            result = (TObjectByteHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public byte compute (Object key,
    			BiFunction<? super Object, ? super Byte, ? extends Byte> remappingFunction)	{
    	int index = index (key);
    	Byte value = null;
    	if (index > 0)	value = remappingFunction.apply((Object)key, (Byte)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public byte comptueIfAbsent (Object key, Function<? super Object,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public byte computeIfPresent (Object key,
    				BiFunction<? super Object, ? super Byte, ? extends Byte> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TObjectByteMap that;
    	int min, max;
    	Equals1 (TObjectByteMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TObjectByteMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
     ...
     */
    public void forEach(BiConsumer<? super Object, ? super Byte> action) {
        //Node<K,V>[] tab;
        if (action == null)
            throw new NullPointerException();
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Object, ? super Byte> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Object, ? super Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Object, ? super Byte> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TObjectByteHashMapSpliterator {
        final TObjectByteHashMap map;
        ...
        TObjectByteHashMapSpliterator(TObjectByteHashMap m, int origin,
                           int fence, int est, Object no_entry_key, byte no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TObjectByteKeySpliterator
        extends TObjectByteHashMapSpliterator
        implements TObjectSpliterator {
        TObjectByteKeySpliterator(TObjectByteHashMap m, int origin, int fence, int est, Object no_entry, byte no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TObjectByteKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TObjectByteKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TObjectByteValueSpliterator
        extends TObjectByteHashMapSpliterator
        implements TByteSpliterator {
        TObjectByteValueSpliterator(TObjectByteHashMap m, int origin, int fence, int est,
                         Object no_entry_key, byte no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TObjectByteValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TObjectByteValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TObjectSet {
        ...
        public TObjectIterator iterator() {
            return new TObjectByteKeyHashIterator (TObjectByteHashMap.this);
        }
        
        /** {@inheritDoc} */
        public TObjectSpliterator spliterator() {
        	return new TObjectByteKeySpliterator(TObjectByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean contains (Object entry) {
            return TObjectByteHashMap.this.contains(entry);
        }
        ...
        public Object[] toArray() {
            return TObjectByteHashMap.this.keys();
        }

        /** {@inheritDoc} */
        public Object[] toArrayPar (int numProcs) {
            return TObjectByteHashMap.this.keysPar(numProcs);
        }

        /** {@inheritDoc} */
        public Object[] toArray (Object[] dest) {
            return TObjectByteHashMap.this.keys(dest);
        }
        public Object[] toArrayPar (Object[] dest, int numProcs)	{
        	return TObjectByteHashMap.this.keysPar(dest, numProcs);
        }


        /**
         * Unsupported when operating upon a Key Set view of a TObjectByteMap
         * <p/>
         * {@inheritDoc}
         */
        ...
        public boolean remove (Object entry) {
            return no_entry_value != TObjectByteHashMap.this.remove(entry);
        }
        /** {@inheritDoc} */
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
                    ...
                    if (array[i] instanceof Object) {
                    	...
                    	Object ele = array[i];
                        if (!TObjectByteHashMap.this.containsKey(ele)) {
                            boolGlobal = false;
                            return;
                        }
                    } 
        ...
        public boolean containsAll (Collection<?> collection) {
            for (Object element : collection) {
                if (element instanceof Object) {
                    ...
                    if (!TObjectByteHashMap.this.containsKey(element)) {
                        return false;
                    }
                } else {
                    return false;
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectByteHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (TObjectCollection collection) {
            ...
            while (iter.hasNext()) {
                if (!TObjectByteHashMap.this.containsKey(iter.next())) {
                    return false;
                }
            }
            return true;
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TObjectByteHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        public boolean containsAll (Object[] array) {
            for (Object element : array) {
                if (!TObjectByteHashMap.this.contains(element)) {
                    return false;
                }
            }
            return true;
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TObjectByteHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TObjectByteHashMap.this.index((Object)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TObjectByteHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Object> action)	{
        	TObjectByteHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Object> action, int numProcs)	{
        	TObjectByteHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public byte merge (Object key, byte value, BiFunction<? super Object, ? super Byte,
    		? extends Byte> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public byte putPar (Object key, byte value) {
     ...
     */
    private byte doPut (Object key, byte value, int index) {
      ...
      * with key
      */
     public byte putParAll (Object key, byte value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private byte doPutAll(Object key, byte value, int index) {
        byte previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    /** {@inheritDoc} */
    public void putAll (Map<? extends Object, ? extends Byte> map) {
        ensureCapacity(map.size());
        // could optimize this for cases when map instanceof THashMap
        for (Map.Entry<? extends Object, ? extends Byte> entry : map.entrySet()) {
            //this.put(entry.getKey().ObjectValue(), entry.getValue().byteValue());
        	this.put(entry.getKey(), entry.getValue().byteValue());
        }
    }
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Object, ? extends Byte> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	Object[] keys;
     	byte[] values;
     	PutAll2 (int a, int b, Object[] c, byte[] d)	{
     ...
     /** {@inheritDoc} */
     public void putAll (TObjectByteMap map) {
         ensureCapacity(map.size());
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TObjectByteMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (Object[] keys, byte[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Object[] keys, byte[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    /** {@inheritDoc} */
    public byte putIfAbsent (Object key, byte value) {
        int index = insertKey(key);
        if (index < 0)
    ...
    /** {@inheritDoc} */
    public byte replace (Object key, byte value)	{
    	int index = index (key);
    	if (index > 0)	{
    ...
    /** {@inheritDoc} */
    public boolean replace (Object key, byte oldValue, byte newValue)	{
    	int index = index (key);
    ...
    public void replaceAll (BiFunction 
    			<? super Object, ? super Byte, ? extends Byte> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	int min, max;
    	BiFunction <? super Object, ? super Byte, ? extends Byte> function;
    	ReplaceAll1 (int a, int b, BiFunction <? super Object, ? super Byte,
			? extends Byte> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Object, ? super Byte,
    ...
    protected class TValueView implements TByteCollection {
        ...
        public TByteIterator iterator() {
            return new TObjectByteValueHashIterator(TObjectByteHashMap.this);
        }

        /** {@inheritDoc} */
        public TByteSpliterator spliterator() {
        	return new TObjectByteValueSpliterator(TObjectByteHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(byte entry, int numProcs) {
            return TObjectByteHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public byte[] toArray() {
            return TObjectByteHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public byte[] toArrayPar(int numProcs) {
            return TObjectByteHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public byte[] toArray(byte[] dest) {
            return TObjectByteHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public byte[] toArrayPar(byte[] dest, int numProcs) {
            return TObjectByteHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	int numElemProc = collection.size()/numProcs;
        	Object[] array = collection.toArray (new Object[collection.size()]);
        	byte[] values = TObjectByteHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Byte> action)	{
        	TObjectByteHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Byte> action, int numProcs)	{
        	TObjectByteHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    class TObjectByteKeyHashIterator extends THashPrimitiveIterator implements TObjectIterator {
         ...
         */
        TObjectByteKeyHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	byte[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, Object[] oldK, byte[] oldV, byte[] oldS){
    ...
    /** {@inheritDoc} */
    public byte adjustOrPutValue (Object key, byte adjust_amount, byte put_amount) {
        int index = insertKey(key);
    ...
    class TObjectByteValueHashIterator extends THashPrimitiveIterator implements TByteIterator {
         ...
         * @param hash the <tt>TPrimitiveHash</tt> we will be iterating over.
         */
        TObjectByteValueHashIterator (TPrimitiveHash hash) {
            super(hash);
        }
    ...
    class TObjectByteHashIterator extends THashPrimitiveIterator implements TObjectByteIterator {

        /**
         * Creates an iterator over the specified map
         *
         * @param map the <tt>TObjectByteHashMap</tt> we will be iterating over.
         */
        TObjectByteHashIterator (TObjectByteHashMap map) {
            super(map);
        }
