...
public class TIntShortHashMap extends TIntShortHash implements TIntShortMap, Serializable, Cloneable {
     ...
     */
    public TIntShortHashMap (int[] keys, short[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TIntShortHashMap(TIntShortMap map, int numProcs) {
        super(map.size());
        if (map instanceof TIntShortHashMap) {
            TIntShortHashMap hashmap = (TIntShortHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TIntShortHashMap clone() {
        TIntShortHashMap result;
        try {
            result = (TIntShortHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TIntShortHashMap clonePar(int numProcs) {
        TIntShortHashMap result;
        try {
            result = (TIntShortHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public short compute (int key,
    			BiFunction<? super Integer, ? super Short, ? extends Short> remappingFunction)	{
    	int index = index (key);
    	Short value = null;
    	if (index > 0)	value = remappingFunction.apply((Integer)key, (Short)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public short comptueIfAbsent (int key, Function<? super Integer,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public short computeIfPresent (int key,
    				BiFunction<? super Integer, ? super Short, ? extends Short> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TIntShortMap that;
    	int min, max;
    	Equals1 (TIntShortMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TIntShortMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Integer, ? super Short> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Integer, ? super Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Integer, ? super Short> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TIntShortHashMapSpliterator {
        final TIntShortHashMap map;
        ...
        TIntShortHashMapSpliterator(TIntShortHashMap m, int origin,
                           int fence, int est, int no_entry_key, short no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TIntShortKeySpliterator
        extends TIntShortHashMapSpliterator
        implements TIntSpliterator {
        TIntShortKeySpliterator(TIntShortHashMap m, int origin, int fence, int est, int no_entry, short no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TIntShortKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TIntShortKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TIntShortValueSpliterator
        extends TIntShortHashMapSpliterator
        implements TShortSpliterator {
        TIntShortValueSpliterator(TIntShortHashMap m, int origin, int fence, int est,
                         int no_entry_key, short no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TIntShortValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TIntShortValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TIntSet {
        ...
        public TIntSpliterator spliterator() {
        	return new TIntShortKeySpliterator(TIntShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public int[] toArrayPar (int numProcs) {
            return TIntShortHashMap.this.keysPar(numProcs);
        }
        ...
        public int[] toArrayPar (int[] dest, int numProcs)	{
        	return TIntShortHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntShortHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntShortHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TIntShortHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TIntShortHashMap.this.index((Integer)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TIntShortHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Integer> action)	{
        	TIntShortHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TIntShortHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public short merge (int key, short value, BiFunction<? super Integer, ? super Short,
    		? extends Short> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public short putPar (int key, short value) {
    ...
    private short doPut (int key, short value, int index) {
      ...
      * with key
      */
     public short putParAll (int key, short value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private short doPutAll(int key, short value, int index) {
        short previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Integer, ? extends Short> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Integer, ? extends Short> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	int[] keys;
     	short[] values;
     	PutAll2 (int a, int b, int[] c, short[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TIntShortMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (int[] keys, short[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(int[] keys, short[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Integer, ? super Short, ? extends Short> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Integer, ? super Short,
			? extends Short> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Integer, ? super Short,
    ...
    protected class TValueView implements TShortCollection {
        ...
        public TShortSpliterator spliterator() {
        	return new TIntShortValueSpliterator(TIntShortHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(short entry, int numProcs) {
            return TIntShortHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public short[] toArray() {
            return TIntShortHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public short[] toArrayPar(int numProcs) {
            return TIntShortHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public short[] toArray(short[] dest) {
            return TIntShortHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public short[] toArrayPar(short[] dest, int numProcs) {
            return TIntShortHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	short[] values = TIntShortHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Short> action)	{
        	TIntShortHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Short> action, int numProcs)	{
        	TIntShortHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	short[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, int[] oldK, short[] oldV, byte[] oldS){
