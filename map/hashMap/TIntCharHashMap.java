...
public class TIntCharHashMap extends TIntCharHash implements TIntCharMap, Serializable, Cloneable {
     ...
     */
    public TIntCharHashMap (int[] keys, char[] values, int numProcs) {
        super(Math.max(keys.length, values.length));
     ...
     * @param numProcs number of processors that will be used
     */
    public TIntCharHashMap(TIntCharMap map, int numProcs) {
        super(map.size());
        if (map instanceof TIntCharHashMap) {
            TIntCharHashMap hashmap = (TIntCharHashMap) map;
            this._loadFactor = hashmap._loadFactor;
     ...
     */
    public TIntCharHashMap clone() {
        TIntCharHashMap result;
        try {
            result = (TIntCharHashMap)super.clone();
     ...
     * @return a shallow copy of this map
     */
    public TIntCharHashMap clonePar(int numProcs) {
        TIntCharHashMap result;
        try {
            result = (TIntCharHashMap)super.clone();
        } catch (CloneNotSupportedException e) {
     ...
     */
    public char compute (int key,
    			BiFunction<? super Integer, ? super Character, ? extends Character> remappingFunction)	{
    	int index = index (key);
    	Character value = null;
    	if (index > 0)	value = remappingFunction.apply((Integer)key, (Character)_values[index]);
    	if (value != null)	{
    		put (key, value);
    		return value;
    	}
     ...
     */
    public char comptueIfAbsent (int key, Function<? super Integer,
     ...
     * @return the new value associated with the specified key, or null if none
     */
    public char computeIfPresent (int key,
    				BiFunction<? super Integer, ? super Character, ? extends Character> remappingFunction)	{
    ...
    private class Equals1 implements Runnable	{
    	TIntCharMap that;
    	int min, max;
    	Equals1 (TIntCharMap obj, int a, int b)	{
    ...
    public boolean equalsPar (Object other, int numProcs)	{
    	if (!((other instanceof TIntCharMap)||(other instanceof Map<?,?>)))	{
    		return false;
    	}
		boolGlobal = true;
    ...
    private class ForEachPar1 implements Runnable	{
    	int min, max;
    	BiConsumer<? super Integer, ? super Character> action;
    	ForEachPar1 (int a, int b, BiConsumer<? super Integer, ? super Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void forEachPar (BiConsumer<? super Integer, ? super Character> action, int numProcs)	{
    	if (action == null)
    		throw new NullPointerException();
    	if (_size > 0 && _values != null) {
    ...
    static class TIntCharHashMapSpliterator {
        final TIntCharHashMap map;
        ...
        TIntCharHashMapSpliterator(TIntCharHashMap m, int origin,
                           int fence, int est, int no_entry_key, char no_entry_value) {
            this.map = m;
            this.index = origin;
            this.fence = fence;
            this.est = est;
    ...
    static final class TIntCharKeySpliterator
        extends TIntCharHashMapSpliterator
        implements TIntSpliterator {
        TIntCharKeySpliterator(TIntCharHashMap m, int origin, int fence, int est, int no_entry, char no_value) {
            super(m, origin, fence, est, no_entry, no_value);
        }

        public TIntCharKeySpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentKey != _no_entry_key) ? null :
            	new TIntCharKeySpliterator(map, lo, index = mid, est >>>= 1,
    ...
    static final class TIntCharValueSpliterator
        extends TIntCharHashMapSpliterator
        implements TCharSpliterator {
        TIntCharValueSpliterator(TIntCharHashMap m, int origin, int fence, int est,
                         int no_entry_key, char no_entry_value) {
            super(m, origin, fence, est, no_entry_key, no_entry_value);
        }

        public TIntCharValueSpliterator trySplit() {
            int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
            return (lo >= mid || currentValue != _no_entry_value) ? null :
                new TIntCharValueSpliterator(map, lo, index = mid, est >>>= 1,
                                          _no_entry_key, _no_entry_value);
        }
    ...
    protected class TKeyView implements TIntSet {
        ...
        public TIntSpliterator spliterator() {
        	return new TIntCharKeySpliterator(TIntCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public int[] toArrayPar (int numProcs) {
            return TIntCharHashMap.this.keysPar(numProcs);
        }
        ...
        public int[] toArrayPar (int[] dest, int numProcs)	{
        	return TIntCharHashMap.this.keysPar(dest, numProcs);
        }
        ...
        private class ContainsAll2 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntCharHashMap.this.containsKey (array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class ContainsAll3 implements Runnable	{
        	...
        	public void run ()	{
        		for (int i=min; i<max; i++) {
        			if (!boolGlobal)	return;
                    if (!TIntCharHashMap.this.contains(array[i])) {
                        boolGlobal = false;
                        return;
                    }
                }
        ...
        private class RetainAll2 implements Runnable	{
        	...
        	public void run ()	{
        		...
        		for (int i=min; i<max; i++) {
        			//if (_states[i] == FULL)	{
        			int pos = TIntCharHashMap.this.index(array[i]);
        			if (pos < 0)	{
        ...
        private class RemoveAll0 implements Runnable	{
        	...
        	public void run()	{
        		...
        		for (int i=min; i<max; i++)	{
        			if (_states[i] == FULL)	{
        				int pos = TIntCharHashMap.this.index((Integer)array[i]);
        				if (pos >= 0)	{
        					_states[i] = REMOVED;
        					removed++;
        				}
        ...
        /** {@inheritDoc} */
        public void clearPar(int numProcs)	{
        	TIntCharHashMap.this.clearPar(numProcs);
        }


        /** {@inheritDoc} */
        public void forEach (Consumer<? super Integer> action)	{
        	TIntCharHashMap.this.forEachKey (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Integer> action, int numProcs)	{
        	TIntCharHashMap.this.forEachKeyPar (action, numProcs);
        }
    ...
    /** {@inheritDoc} */
    public char merge (int key, char value, BiFunction<? super Integer, ? super Character,
    		? extends Character> remappingFunction)	{
    	if (value == no_entry_value)
    		throw new NullPointerException();
    	if (remappingFunction == null)
     ...
     * with key
     */
     public char putPar (int key, char value) {
    ...
    private char doPut (int key, char value, int index) {
      ...
      * with key
      */
     public char putParAll (int key, char value) {
     	//System.out.println ("Put: " + key + ", " + value);
     ...
     * associated null with key
     */
    private char doPutAll(int key, char value, int index) {
        char previous = no_entry_value;
        //boolean isNewMapping = true;
        if (index < 0) {
    ...
    public void putAll (Map<? extends Integer, ? extends Character> map) {
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(Map<? extends Integer, ? extends Character> map, int numProcs) {
         ensureCapacity(map.size());
         atIntGlobal = new AtomicInteger(0);
     ...
     private class PutAll2 implements Runnable	{
     	...
     	int[] keys;
     	char[] values;
     	PutAll2 (int a, int b, int[] c, char[] d)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar (TIntCharMap map, int numProcs) {
         ensureCapacity(map.size());
         int numMin, numMax;
         threads = new Thread[numProcs];
     ...
     /** {@inheritDoc} */
     public void putAll (int[] keys, char[] values)	{
      ...
      * @param numProcs number of threads that will be used
      */
     public void putAllPar(int[] keys, char[] values, int numProcs) {	
     	int size = keys.length;
         ensureCapacity (size);
         _size+=size;
         _free-=size;
    ...
    public void replaceAll (BiFunction 
    			<? super Integer, ? super Character, ? extends Character> function)	{
    	for (int i=0; i<_values.length; i++)	{
    		if (_states[i] == FULL)	{
    ...
    private class ReplaceAll1 implements Runnable	{
    	...
    	ReplaceAll1 (int a, int b, BiFunction <? super Integer, ? super Character,
			? extends Character> c)	{
     ...
     * @param numProcs number of threads that will be used
     */
    public void replaceAllPar (BiFunction <? super Integer, ? super Character,
    ...
    protected class TValueView implements TCharCollection {
        ...
        public TCharSpliterator spliterator() {
        	return new TIntCharValueSpliterator(TIntCharHashMap.this, 0, -1, 0, no_entry_key, no_entry_value);
        }
        ...
        public boolean containsPar(char entry, int numProcs) {
            return TIntCharHashMap.this.containsValuePar(entry, numProcs);
        }


        /** {@inheritDoc} */
        public char[] toArray() {
            return TIntCharHashMap.this.toArrayValues();
        }

        /** {@inheritDoc} */
        public char[] toArrayPar(int numProcs) {
            return TIntCharHashMap.this.toArrayValuesPar(numProcs);
        }

        /** {@inheritDoc} */
        public char[] toArray(char[] dest) {
            return TIntCharHashMap.this.toArrayValues(dest);
        }
        /** {@inheritDoc} */
        public char[] toArrayPar(char[] dest, int numProcs) {
            return TIntCharHashMap.this.toArrayValuesPar(dest, numProcs);
        }
        ...
        public boolean containsAllPar (Collection<?> collection, int numProcs)	{
        	...
        	char[] values = TIntCharHashMap.this.values().toArrayPar(numProcs);
        	Arrays.sort(values);
        	int numMin, numMax;
            threads = new Thread[numProcs];
        	for (int i=0; i<numProcs; i++)	{
        ...
        public void forEach (Consumer<? super Character> action)	{
        	TIntCharHashMap.this.forEachValue (action);
        }
        /** {@inheritDoc} */
        public void forEachPar (Consumer<? super Character> action, int numProcs)	{
        	TIntCharHashMap.this.forEachValuePar (action, numProcs);
        }
    ...
    private class Rehash1 implements Runnable	{
    	...
    	char[] oldValues;
    	byte[] oldStates;
    	Rehash1 (int a, int b, int[] oldK, char[] oldV, byte[] oldS){
